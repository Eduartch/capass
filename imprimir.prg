...
********************
	...
	If Type('oempresa') = 'U' Then
		If !Directory(Sys(5) + Sys(2003) + "\pdf") Then
			ccarpeta = Sys(5) + Sys(2003) + "\pdf"
			Mkdir (ccarpeta)
		Endif
		cuta = Addbs(Sys(5) + Sys(2003) + '\pdf')
		carchivopdf = Addbs(Sys(5) + Sys(2003) + '\PDF') + filepdf
	Else
		If !Directory(Sys(5) + Sys(2003) + "\pdf\" + Alltrim(Oempresa.nruc)) Then
			ccarpeta = Sys(5) + Sys(2003) + "\pdf\" + Alltrim(Oempresa.nruc)
			Mkdir (ccarpeta)
		Endif
		cuta = Addbs(Sys(5) + Sys(2003) + '\pdf\' + Alltrim(Oempresa.nruc))
		carchivopdf  = Addbs(Sys(5) + Sys(2003) + '\PDF\' + Alltrim(Oempresa.nruc)) + filepdf
	Endif
	cpropiedad = "Impresoranormal"
	If !Pemstatus(goApp, cpropiedad, 5)
		goApp.AddProperty("Impresoranormal", "")
