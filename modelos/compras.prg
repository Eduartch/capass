Define Class Compras As OData Of 'd:\capass\database\data.prg'
	...
	Endif
	TEXT To lC Noshow Textmerge
	  SELECT   `a`.`idauto`    AS `idauto`,  `a`.`alma`      AS `alma`,  `a`.`idkar`     AS `idkar`,
	  `a`.`kar_equi`  AS `equi`,  `b`.`descri`    AS `descri`,  `b`.`peso`      AS `peso`,
	  `b`.`prod_idco` AS `prod_idco`,  `b`.`unid`      AS `unid`,  `b`.`tipro`     AS `tipro`,
	  `a`.`idart`     AS `idart`,  `a`.`kar_chek`  AS `kar_chek`,  `a`.`incl`      AS `incl`,
	  `c`.`ndoc`      AS `ndoc`,  `c`.`valor`     AS `valor`,  `c`.`igv`       AS `igv`,
	  `c`.`impo`      AS `impo`,  `c`.`pimpo`     AS `pimpo`,  `a`.`cant`      AS `cant`,
	  `a`.`prec`      AS `prec`,  `c`.`fech`      AS `fech`,  `c`.`fecr`      AS `fecr`,
	  ...
	  `p`.`nruc`      AS `nruc`,  IFNULL(`x`.`idcaja`,0) AS `Idcaja`,  `c`.`codt`      AS `codt`,
	  `a`.`dsnc`      AS `dsnc`,  `a`.`dsnd`      AS `dsnd`,  `a`.`gast`      AS `gast`,  `c`.`fusua`     AS `fusua`,  `w`.`nomb`      AS `Usuario`
	FROM `fe_rcom` `c`
	     LEFT JOIN `fe_kar` `a`           ON `c`.`idauto` = `a`.`idauto`
	     LEFT JOIN `fe_art` `b`          ON `b`.`idart` = `a`.`idart`
	     JOIN `fe_prov` `p`         ON `p`.`idprov` = `c`.`idprov`
	     ...
	     JOIN `fe_usua` `w`       ON `w`.`idusua` = `c`.`idusua`
	     JOIN `fe_sucu` `s`      ON `s`.`idalma` = `c`.`codt`
	...
	ofechas = dfianddff(This.nmes, This.Naño)
	f1 = Cfechas(ofechas.dfi)
	f2 = Cfechas(ofechas.dff)
	Set Textmerge On
	Set Textmerge To  Memvar lC Noshow Textmerge
	\Select a.Auto, a.fech, a.Fevto, b.Tdoc, If(Length(Trim(a.Ndoc)) <= 10, Left(a.Ndoc, 3), Left(a.Ndoc, 4)) As Serie,
	\If(Length(Trim(a.Ndoc)) <= 10, MID(a.Ndoc, 4, 7), MID(a.Ndoc, 5, 8)) As Ndoc, d.nruc,d.ndni, d.Razo,
    \	Sum(Case c.Nitem When 1 Then If(a.Mone = 'S', c.Impo, Round(c.Impo * a.dolar, 2)) Else 0 End) As v1,
	\	Sum(Case c.Nitem When 2 Then If(a.Mone = 'S', c.Impo, Round(c.Impo * a.dolar, 2)) Else 0 End) As v2,
	\	Sum(Case c.Nitem When 3 Then If(a.Mone = 'S', c.Impo, Round(c.Impo * a.dolar, 2)) Else 0 End) As v3,
	\	Sum(Case c.Nitem When 4 Then If(a.Mone = 'S', c.Impo, Round(c.Impo * a.dolar, 2)) Else 0 End) As v4,
	...
	\	IFNULL(w.Tdoc, '00') As tref, e.fech As fechn, IFNULL(e.idrefe, 0) As idrefe,a.idrcon,
	...
	\Where a.rcon_acti = 'A'  And fecr Between '<<f1>>' And '<<f2>>'
	If This.nidt > 0 Then
	...
	\Y.cant,Y.Prec,Round(Y.cant*Y.Prec,2)As Importe,dsnc,dsnd,gast,IFNULL(Y.idart,0) As idart,
	\IFNULL(z.Descri,'Sin Productos      ') As Descri,IFNULL(z.Unid,'') As Unid,w.nomb As Usuario,x.fusua,T.nruc,T.Razo,'' As dni,d.nomb From fe_rcom x
	\INNER Join fe_prov T On T.idprov=x.idprov
	\INNER Join fe_usua w On w.idusua=x.idusua
	\INNER Join fe_sucu As d On(d.idalma=x.codt)
	\Left Join
...
*This.cTdoc, Left(This.cforma, 1), This.cndoc, This.dFecha, This.dfechar, This.Cdetalle, Nv, nigv, Nt, This.cguia, This.Cmoneda, ndolar, Tigv, '1', This.nidprov, '1', goApp.nidusua, 0, This.codt, nidcta1, nidcta2, nidcta3, 0, This.npercepcion)
	...
	Return 1
	ENDFUNC
	Function listaromprasparasaplicarnotas(Ccursor)
	If This.Idsesion > 1 Then
		Set DataSession To This.Idsesion
	Endif
	Set Textmerge On
    ...
    \Select  Tdoc,Ndoc,fech,b.Razo,Mone,valor,igv,Impo,idauto,a.idprov As cod,fecr From
    \fe_rcom As a
    \INNER Join fe_prov As b On b.idprov=a.idprov Where a.Acti<>'I' And Tdoc In("01")
	If This.nidprov > 0 Then
       \ And a.idprov=<<This.nidprov>>
	Else
		If This.nmes > 0 Then
	...
	\c.Ndoc As Ndoc,Month(c.fech) As mes,s.nomb As Tienda,c.Mone,a.alma,a.Prec As preciocompra,c.idprov,a.Prec As costo,
	\Cast(If(c.Mone='D',Prec*c.dolar*c.vigv,Prec*c.vigv) As Decimal(12,6))As precios From fe_kar As a
	\INNER Join fe_rcom  As c On(c.idauto=a.idauto)
	...
	octasxpagar = Newobject("ctasporpagar", "d:\capass\modelos\ctasxpagar.prg")
	If octasxpagar.Verificaestadodeuda(This.Nreg) < 1 Then
		This.Cmensaje = octasxpagar.Cmensaje
		Return 0
	Endif
	octasxpagar = Null
	Return 1
	...
	Case REgdvto(This.ctemporal) = 0
		...
		Return .F.
	Case Len(Alltrim(Left(This.cndoc, 4))) < 4
		This.Cmensaje = "Ingrese Serie de Documento Válido"
		Return .F.
	Case  Len(Alltrim(Substr(This.cndoc, 5))) < 8
		This.Cmensaje = "Ingrese Número de Documento Válido"
		Return .F.
	Case This.cencontrado = "V"
		This.Cmensaje = "NO es Posible Actualizar este Documento"
		Return .F.
	Case Month(This.dfechar) <> goApp.mes
		This.Cmensaje = "Mes NO Permitido Por el Sistema"
		Return .F.
	Case Year(This.dfechar) <> Val(goApp.Año)
		This.Cmensaje = "Año NO Permitido Por el Sistema"
		Return .F.
	Case  !esfechaValida(This.dFecha)
		This.Cmensaje = "Fecha de Emisión no Válida"
		Return .F.
	Case  !esfechaValida(This.dfechar)
		This.Cmensaje = "Fecha de Registro  no Válida"
		Return .F.
	Case This.nidprov < 1
	...
	\From fe_rcom r INNER Join fe_kar k On k.idauto=r.idauto Where k.Acti='A' And r.Acti='A'  And Year(fech)=<<This.Naño>> And r.idprov>0
	Do Case
	Case m.ntipo = 2
	     \ And r.Tdoc In("01","03,"07","08","II","50")
	Case m.ntipo = 3
	    \ And r.Tdoc In("AJ",'GI')
	Otherwise
	    \
	...
	\From fe_rcom r
	\INNER Join fe_kar k On k.idauto=r.idauto Where k.Acti='A' And r.Acti='A'  And Year(fech)=<<This.Naño>> And r.idprov>0
	Do Case
	Case m.ntipo = 2
	     \ And r.Tdoc In("01","03,"07","08","II","50")
	Case m.ntipo = 3
	...
	Return 1
	Endfunc
	Function AnalisisporMarca(Ccursor)
	If This.Idsesion > 0 Then
		Set DataSession To This.Idsesion
	Endif
	TEXT To lC Noshow Textmerge
	...
	sum(case mes when 11 then impo else 0 end) as 'Noviembre',
	sum(case mes when 12 then impo else 0 end) as 'Diciembre',sum(impo) as Total
    from (select MONTH(fech) as mes,c.idmar,cast(sum(if(mone='S',cant*k.prec,cant*k.prec*dolar)) as decimal(12,2)) as Impo
	...
	Return 1
	Endfunc
	Function AnalisisporLinea(Ccursor)
	If This.Idsesion > 0 Then
		Set DataSession To This.Idsesion
	Endif
	TEXT To lC Noshow Textmerge
	...
	Return 1
	Endfunc
	Function registrocomprasNodomiciliados(Ccursor)
	If This.Idsesion > 0 Then
		Set DataSession To This.Idsesion
	Endif
	TEXT  To lC Noshow Textmerge
			...
			where com1_ActI='A' and MONTH(com1_fecr)=<<this.nmes>> and YEAR(com1_fecr)=<<this.Naño>>
	ENDTEXT
	If This.EJECutaconsulta(lC, m.Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
...
*NEXT
	...
	Ccursor = 'c_' + Sys(2015)
	ofechas = dfianddff(m.nmes, m.na)
	dfi = cfechas(ofechas.dfi)
	dff = cfechas(ofechas.dff)
	TEXT To lC Noshow
		select a.idauto as clave,a.idauto,
		ROUND(IF(a.tdoc='07',IFNULL(rr.dolar,0),IF(a.tdoc='08',IFNULL(rr.`dolar`,0),x.venta)),3) AS tc
		FROM  fe_rcom as a
		inner join fe_mon as x on x.fech=a.fech
		LEFT JOIN fe_nccom AS n ON ncre_idan=a.idauto
	    LEFT JOIN fe_rcom AS rr ON rr.`idauto`=n.`ncre_idau`
	    where a.acti='A' and  a.fech between '<<dfi>>' and '<<dff>>' and a.idprov>0
	ENDTEXT
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	Ccursor = 'c_' + Sys(2015)
	lC = "FUNVALIDADCTOSCOMPRAS"
	TEXT To lp Noshow
     (?nauto)
	ENDTEXT
	xd = This.EJECUTARf(lC, lp, Ccursor)
	If This.conerror = 1 Then
		Return 0
	Endif
	If xd > 0 Then
		This.Cmensaje = 'No Es Posible Anular Este Documento Tiene Relaciòn Con Compras al Crèdito o Tiene Costos en Documentos de Ventas'
		Return 0
	Endif
	If PermiteIngresoACaja(dFecha) < 1 Then
		This.Cmensaje = 'La Caja de Esta Fecha Esta Liquidada'
		Return  0
	Endif
	This.Nreg = m.NAuto
	If This.Verificaestadodeuda() < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function PermiteIngresoCompras(cndoc, cTdoc, nidpr, nidauto, dFecha)
	objdcto = Createobject("empty")
	AddProperty(objdcto, 'cdcto', m.cndoc)
	AddProperty(objdcto, 'ctdoc', m.cTdoc)
	AddProperty(objdcto, 'idp', m.nidpr)
	This.Nreg = m.nidauto
	If This.haycompra(objdcto) < 1 Then
		Return 0
	Endif
	If This.Verificaestadodeuda() < 1 Then
		Return 0
