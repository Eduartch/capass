...
*:Global cdesc, nidkar, s, sw
	...
	Do While !Eof()
		...
		Else
			If tmpvg.Nreg = 0 Then
				If Trim(This.Proyecto) = 'psysw' Or Trim(This.Proyecto) = 'xmsys' Or Trim(This.Proyecto) = 'psysm' Or Trim(This.Proyecto) = 'psystr' Then
					nidkar = INGRESAKARDEX1(This.Idauto, tmpvg.Coda, 'V', tmpvg.Prec, tmpvg.cant, 'I', 'K', 0, goApp.Tienda, 0)
				Else
					nidkar = IngresaKardexGrifo(This.Idauto, tmpvg.Coda, 'V', tmpvg.Prec, tmpvg.cant, 'I', 'K', 0, goApp.Tienda, 0, 0)
				Endif
				If nidkar = 0 Then
					Sw			  = 0
					This.Cmensaje = "Al Registrar Producto - " + Alltrim(cdesc)
			...
			_Screen.oproductos.ncaant = tmpvg.caant
			If _Screen.oproductos.ActualizaStock1() < 1 Then
				Sw			  = 0
...
*!*			wait WINDOW this.constancia
	...
	Case Empty(This.ubigeocliente) And This.Tdoc = '09'
		This.Cmensaje = "Ingrese el Ubigeo"
		Return 0
	Otherwise
		Return 1
	Endcase
	...
	m.ndire=0
	If This.Iddireccion>0 Then
		m.ndire=This.Iddireccion
	Endif
	If This.IniciaTransaccion() = 0 Then
		Return 0
	Endif
	NAuto = IngresaResumenDcto('09', 'E', This.Ndoc, This.Fecha, This.Fecha, "", 0, 0, 0, '', 'S', fe_gene.dola, fe_gene.igv, 'k', This.Codigo, 'V', goApp.nidusua, 1, goApp.Tienda, 0, 0, 0, m.ndire, 0)
	If NAuto < 1 Then
		This.DEshacerCambios()
		Return 0
	Endif
	...
	Do While !Eof()
		...
		If VerificaAlias("tramos") = 1
			...
			Endif
		Endif
		If _Screen.oproductos.ActualizaStock(tmpvg.Coda, goApp.Tienda, tmpvg.cant, 'V') < 1 Then
			s = 0
			This.Cmensaje = _Screen.oproductos.Cmensaje
			Exit
		Endif
	...
	Else
		Cruc = Oempresa.nruc
		If Alltrim(goApp.Empresanube) = 'rgm' Or Alltrim(goApp.Empresanube) = 'fminera' Or Alltrim(goApp.Empresanube)='kya'  Then
			Cmulti = 'S'
		Else
			Cmulti = ""
		Endif
...
*!*		MESSAGEBOX(lcHTML)
	...
	Case This.Motivo = 'V'
		If goApp.Cdatos <> 'S' Then
			If goApp.Proyecto = 'psys3' Then
		        ...
		        e.entr_idar AS coda,g.guia_idtr,placa AS placa,t.razon AS razont,
		        t.ructr AS ructr,t.nombr AS conductor,t.dirtr AS direcciont,t.breve AS brevete,t.cons AS constancia,t.marca AS marca,c.nruc,c.ndni,
		        t.placa1 AS placa1,r.ndoc AS dcto,'09' as tdoc,r.idcliente,v.gene_usol,v.gene_csol,guia_ubig,guia_deta,
	...
	Case This.Motivo = 'I'
		If goApp.Cdatos = 'S' Then
            ...
            a.descri,IFNULL(unid_codu,'NIU')AS unid,e.entr_cant AS cant,a.peso,g.guia_ptoll AS ptollegada,
            a.idart AS  coda,g.guia_idtr,IFNULL(placa,'') AS placa,IFNULL(t.razon,'') AS razont,
            t.ructr AS ructr,t.nombr AS conductor,
            t.dirtr AS direcciont,t.breve AS brevete,
            IFNULL(t.cons,'') AS constancia,IFNULL(t.marca,'') AS marca,
            IFNULL(t.placa1,'') AS placa1,'09' AS tdoc,v.gene_usol,v.gene_csol,guia_ubig,'' AS tdoc1,'' AS dcto,guia_deta,
            guia_idgui AS idgui,v.rucfirmad,gene_cert,clavecertificado AS clavecerti,guia_moti,clavecertificado,
            v.razonfirmad,v.nruc AS rucempresa,v.nomb AS empresa,v.ubigeo,g.guia_ptop AS ptop,v.ciud AS ciudad,v.distrito,t.tran_tipo
            FROM  fe_guias AS g
            INNER JOIN fe_ent AS e ON e.entr_idgu=g.guia_idgui
            INNER JOIN fe_art AS a ON a.idart=e.entr_idar
            INNER JOIN fe_sucu AS v ON v.idalma=g.guia_codt
            LEFT JOIN fe_unidades AS u ON u.unid_codu=a.unid
            INNER JOIN fe_tra AS t ON t.idtra=g.guia_idtr WHERE guia_idgui=<<this.Idautog>> AND entr_acti='A'
			ENDTEXT
		Endif
	Case This.Motivo = 'O'
		TEXT To lC Noshow Textmerge
	        ...
	        LEFT(guia_ndoc,4) AS serie,SUBSTR(guia_ndoc,5) AS numero,
	        a.descri,IFNULL(unid_codu,'NIU')AS unid,e.entr_cant AS cant,a.peso,g.guia_ptoll AS ptollegada,
	        k.idart AS coda,g.guia_idtr,placa,t.razon AS razont,
	        t.ructr AS ructr,t.nombr AS conductor,t.dirtr AS direcciont,t.breve AS brevete,
	        t.cons AS constancia,t.marca AS marca,c.nruc,c.ndni,
	        t.placa1,'' AS dcto,'09' AS tdoc,v.gene_usol,v.gene_csol,guia_ubig,
	        ...
	        LEFT(guia_ndoc,4) AS serie,SUBSTR(guia_ndoc,5) AS numero,
	        a.descri,IFNULL(unid_codu,'NIU')AS unid,e.entr_cant AS cant,a.peso,g.guia_ptoll AS ptollegada,
	        entr_idar AS coda,g.guia_idtr,placa,t.razon AS razont,
	        t.ructr AS ructr,t.nombr AS conductor,t.dirtr AS direcciont,t.breve AS brevete,
	        t.cons AS constancia,t.marca AS marca,v.nruc,'',
	        t.placa1,'' AS dcto,'09' AS tdoc,v.gene_usol,v.gene_csol,guia_ubig,
	        v.empresa as razo,guia_idgui AS idgui,CAST(0 as unsigned) as idauto,v.ptop as dire,v.ciudad as ciud,'' AS tdoc1,
	        v.rucfirmad,gene_cert,clavecertificado AS clavecerti,guia_moti,guia_deta,
	        v.razonfirmad,v.nruc AS rucempresa,v.empresa,v.ubigeo,g.guia_ptop AS ptop,v.ciudad,v.distrito,t.tran_tipo
	        FROM  fe_guias AS g
	        INNER JOIN fe_ent AS e ON e.entr_idgu=g.guia_idgui
	        INNER JOIN fe_art AS a ON a.idart=e.entr_idar
...
* MESSAGEBOX(cdata)
	...
	Create Cursor (Calias)(Coda c(15), Descri c(80), Unid c(6), cant N(10, 2), Prec N(10, 2), uno N(10, 2), Dos N(10, 2), lote c(15), ;
		...
		Motivo c(1), Detalle c(100), tipotra c(15), Remitente c(150), rucremitente c(11))
	Select (Calias)
	...
	Create Cursor (Calias)(Coda N(8), duni c(20), Descri c(120), Unid c(20), cant N(10, 2), Prec N(10, 5), uno N(10, 2), Dos N(10, 2), lote c(15), ;
		...
		Remitente c(150), rucremitente c(11),Idauto N(10))
	Select (Calias)
	Index On Descri Tag Descri
	Index On Nitem Tag Items
	Endfunc
	...
	cotrasguias=''
	SELECT fe_gene
	IF FSIZE("gene_ogui")>0 then
	   cotrasguias='S'
	ENDIF 
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	    \      Select  fech,Ndoc,cliente,Detalle,Refe,Transportista,idguia,guia_arch,clie_corr,Motivo,guia_codt,guia_otro From(
        \      Select guia_fech As fech,guia_ndoc As Ndoc,c.Razo As cliente,guia_deta As Detalle,r.Ndoc As Refe,
		\      T.razon As Transportista,guia_idgui As idguia,guia_arch,clie_corr,'V' As Motivo,guia_codt,'' as guia_otro  From
		\      fe_guias As g
		\      INNER Join fe_rcom As r On r.Idauto=g.guia_idau
		\      INNER Join fe_clie As c On c.idclie=r.Idcliente
	...
	Endif
	    ...
	    \      T.razon As Transportista,guia_idgui As idguia,guia_arch,email As clie_corr,'D' As Motivo,guia_codt,'' as guia_otro  From
	    \      fe_guias As g
	...
	Endif
        \      Union All
        \      Select guia_fech As fech,guia_ndoc As Ndoc,p.Razo As cliente,guia_deta As Detalle,guia_dcto As Refe,
	    \      T.razon As Transportista,guia_idgui As idguia,guia_arch,email As clie_corr,'C' As Motivo,guia_codt 
	    IF m.cotrasguias='S' then
	       \,guia_otro
	      ELSE 
	        \,'' as guia_otro 
	    ENDIF  
	...
	Endif
        ...
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'T' As Motivo,guia_codt,'' as guia_otro  From fe_guias As a
	...
	Endif
	    \      Union All
        \      Select guia_fech As fech,guia_ndoc As Ndoc,g.Empresa As cliente,guia_deta As Detalle,guia_dcto As Refe,
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'O' As Motivo,guia_codt,'' as guia_otro  From fe_guias As a
        \      INNER Join fe_tra As T On T.idtra=a.guia_idtr,fe_gene  As g
	...
	Endif
		...
		\	T.razon As Transportista, guia_idgui As idguia, guia_arch, clie_corr, guia_moti As Motivo, guia_codt,'' as guia_otro  From fe_guias As a
		\	INNER Join fe_tra As T On T.idtra = a.guia_idtr
		\	INNER Join fe_rcom As r On r.Idauto = a.guia_idau
		\	INNER Join fe_clie As c On c.idclie = r.Idcliente
		\	Where Left(guia_ndoc, 1) = 'T'  And guia_fech Between '<<dfi>>' And '<<dff>>' And guia_moti = 'N' And guia_acti = 'A'
	If  nidt > 0 Then
        \And guia_codt=<<nidt>>
	Endif
        \      Union All
        \      Select guia_fech As fech,guia_ndoc As Ndoc,'' As cliente,guia_deta As Detalle,guia_dcto As Refe,
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'I' As Motivo,guia_codt,'' as guia_otro  From fe_guias As a
        \      INNER Join fe_tra As T On T.idtra=a.guia_idtr,fe_gene  As g
	...
	Endif
       \ ) As w
        \      Group By fech,Ndoc,cliente,Detalle,Refe,Transportista,idguia,guia_arch,clie_corr,Motivo,guia_codt,guia_otro Order By fech
	Set Textmerge Off
	Set Textmerge To
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	\Select lgui_fech As hora, u.`nomb` As Usuario,guia_ndoc As NroGuia, guia_fech As Fecha,  s.`nomb` As Tienda,
	\p.`Descri` As Producto, d.`entr_cant` As Cantidad,guia_idgui From fe_aguias As a
	\INNER Join fe_guias As r On r.`guia_idgui` = a.`lgui_idga`
	\INNER Join fe_sucu As s On s.`idalma` = r.`guia_codt`
	\INNER Join fe_usua As u On u.`Idusua` = a.`lgui_idus`
	\Left Join fe_ent As d On d.`entr_idgu` = r.`guia_idgui`
	\Left Join fe_art As p On p.`idart` = d.`entr_idar`
	\Where
	If cfechaanulacion = 'S' Then
	     \ a.lgui_fech Between '<<f1>>' And '<<f2>>'
	Else
	     \  guia_fech  Between '<<f1>>' And '<<f2>>'
	Endif
    \ Order By lgui_fech Desc
	Set Textmerge Off
	Set Textmerge To
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	...
	If TRIM(goApp.Proyecto) == 'psysr' Or TRIM(goApp.Proyecto) == 'psys'  OR TRIM(goApp.Proyecto) == 'psysrx' Then
	 \ '' As prod_cod1,
	Else
	 \  prod_cod1,
	Endif
	...
	Return 1
	Endfunc
	Function listarguiasporventas(nmes,na,Ccursor)
	TEXT TO lc NOSHOW TEXTMERGE
           select '09' as tdoc,guia_ndoc as ndoc,guia_fech as fech,
		   c.Razo,'S' as mone,0 as valor,0 as igv,0 as impo,guia_idgui as idauto FROM
		   fe_guias as g
		   inner join fe_rcom as r on r.idauto=g.guia_idau
		   inner join fe_clie as c on c.idclie=r.idcliente
		   where guia_acti='A' and guia_moti='V' and MONTH(guia_fech)=<<nmes>> and YEAR(guia_fech)=<<na>>  ORDER BY fech desc,ndoc
	ENDTEXT
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function listarguiasporeimisorintinerante(nmes,na,Ccursor)
	TEXT TO lc NOSHOW TEXTMERGE
           select '09' as tdoc,guia_ndoc as ndoc,guia_fech as fech,
		   '' as razo,'S' as mone,0 as valor,0 as igv,0 as impo,guia_idgui as idauto FROM
		   fe_guias as g
		   where guia_acti='A' and guia_moti='I' and MONTH(guia_fech)=<<nmes>> and YEAR(guia_fech)=<<na>>  ORDER BY fech desc,ndoc
	ENDTEXT
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	Case This.Motivo = 'V'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,c.Razo As cliente,guia_deta As Detalle,r.Ndoc As Refe,
		\      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_arch,clie_corr,'V' As Motivo,guia_codt,'' as guia_otro  From
		\      fe_guias As g
		\      INNER Join fe_rcom As r On r.Idauto=g.guia_idau
		\      INNER Join fe_clie As c On c.idclie=r.Idcliente
		\      Left Join fe_tra As T On T.idtra=g.guia_idtr
	...
	Case This.Motivo = 'D'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,p.Razo As cliente,guia_deta As Detalle,guia_dcto As Refe,
	    \      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_arch,email As clie_corr,'D' As Motivo,guia_codt,'' as guia_otro  From
	    \      fe_guias As g
		\      INNER Join fe_prov As p On p.idprov=g.guia_idpr
		\      Left Join fe_tra As T On T.idtra=g.guia_idtr
        \      Where Left(guia_ndoc,1)='T'   And guia_fech Between '<<dfi>>' And '<<dff>>'   And guia_moti='D'  And guia_acti='A'
	...
	Case This.Motivo = 'C'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,p.Razo As cliente,guia_deta As Detalle,guia_dcto As Refe,
	    \      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_arch,email As clie_corr,'C' As Motivo,guia_codt
	    IF m.cotrasguias='S' then
	       \,guia_otro
	      ELSE 
	        \,'' as guia_otro 
    ...
    \INNER Join fe_tra As T On T.idtra=g.guia_idtr
    \Where Left(guia_mens,1)<>'0' And Left(guia_ndoc,1)='T' And guia_moti='D' And guia_acti='A' And Left(guia_deta,7)<>'Anulada'
	If goApp.Cdatos = 'S' Then
		If Empty(goApp.Tiendas) Then
	      \And guia_codt=<<goApp.Tienda>>
		Else
    ...
    \INNER Join fe_tra As T On T.idtra=g.guia_idtr
    \Where  Left(guia_mens,1)<>'0' And Left(guia_ndoc,1)='T' And guia_moti='C' And guia_acti='A' And Left(guia_deta,7)<>'Anulada'
	If goApp.Cdatos = 'S' Then
		If Empty(goApp.Tiendas) Then
	      \And guia_codt=<<goApp.Tienda>>
		Else
    ...
    \INNER Join fe_tra As T On T.idtra=g.guia_idtr
    \Where  Left(guia_mens,1)<>'0' And Left(guia_ndoc,1)='T' And guia_moti='N' And guia_acti='A' And Left(guia_deta,7)<>'Anulada'
	If goApp.Cdatos = 'S' Then
		If Empty(goApp.Tiendas) Then
	      \And guia_codt=<<goApp.Tienda>>
		Else
    ...
    \INNER Join fe_tra As T On T.idtra=a.guia_idtr,fe_gene  As g
    \Where Left(guia_ndoc,1)='T' And  Left(guia_mens,1)<>'0' And guia_moti='T' And guia_acti='A' And Left(guia_deta,7)<>'Anulada'
	If goApp.Cdatos = 'S' Then
		If Empty(goApp.Tiendas) Then
	      \And guia_codt=<<goApp.Tienda>>
		Else
    ...
    \INNER Join fe_tra As T On T.idtra=a.guia_idtr,fe_gene  As g
    \Where Left(guia_ndoc,1)='T' And  Left(guia_mens,1)<>'0' And guia_moti='O' And guia_acti='A' And Left(guia_deta,7)<>'Anulada'
	If goApp.Cdatos = 'S' Then
		If Empty(goApp.Tiendas) Then
	      \And guia_codt=<<goApp.Tienda>>
		Else
	...
	Return 1
	Endfunc
	Function listarguiadirectaxid(nids, Ccursor)
	If This.Idsesion > 0 Then
		Set DataSession To This.Idsesion
	Endif
	TEXT To lC Noshow Textmerge
	   select  guia_ndoc AS ndoc,guia_fech AS fech,guia_fect AS fechat,guia_deta AS detalle,
 	   a.descri,a.unid,e.entr_cant AS cant,a.peso,g.guia_ptoll AS ptollegada,
	   ...
	   IFNULL(t.placa1,'') AS placa1,g.guia_ndoc AS dcto,r.idcliente,
	   c.razo,guia_idgui AS idgui,r.idauto,'09' AS tdoc,
	   guia_ptop AS ptop,v.ciudad,v.distrito,IFNULL(t.tran_tipo,'01') AS tran_tipo,guia_ubig
	   FROM  fe_guias AS g
	   INNER JOIN fe_ent AS e ON e.entr_idgu=g.guia_idgui
	   INNER JOIN fe_kar AS k ON k.`idkar`=e.`entr_idkar`
	   INNER JOIN fe_art AS a ON a.idart=k.idart
	   ...
	   WHERE guia_idgui=<<nids>> AND r.tdoc='09' AND entr_acti='A' and k.acti='A'
	ENDTEXT
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function listardetalleguiaspormotivo(nidt, Ccursor)
	If This.dff - This.dfi > 240 Then
		This.Cmensaje = 'Hasta 240 Días'
		Return 0
	Endif
	...
	Case This.Motivo = 'V'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,c.Razo As cliente,guia_deta As Detalle,r.Ndoc As Refe,
		\      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_arch,clie_corr,'V' As Motivo,guia_codt,
		\      entr_cant As cant,a.Descri,a.Unid,s.nomb As Tienda,a.idart,guia_ptoll As ptoll From
		\      fe_guias As g
		\      INNER Join fe_rcom As r On r.Idauto=g.guia_idau
		\      INNER Join fe_clie As c On c.idclie=r.Idcliente
		\      INNER Join fe_ent As e On e.entr_idgu=g.guia_idgui
		...
		If This.Idcliente > 0 Then
		  \  And r.Idcliente=<<This.Idcliente>>
		Endif
	Case This.Motivo = 'D'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,p.Razo As cliente,guia_deta As Detalle,guia_dcto As Refe,
	    \      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_arch,email As clie_corr,'D' As Motivo,guia_codt,
	    \      entr_cant As cant,a.Descri,a.Unid,s.nomb As Tienda,a.idart,guia_ptoll As ptoll From
	    \      fe_guias As g
		\      INNER Join fe_prov As p On p.idprov=g.guia_idpr
		\      Left Join fe_tra As T On T.idtra=g.guia_idtr
	...
	Case This.Motivo = 'T'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,g.Empresa As cliente,guia_deta As Detalle,guia_dcto As Refe,
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'T' As Motivo,guia_codt,
        \      entr_cant As cant,aa.Descri,aa.Unid,s.nomb As Tienda,aa.idart,guia_ptoll As ptoll From fe_guias As a
        \      INNER Join fe_tra As T On T.idtra=a.guia_idtr
        \      INNER Join fe_ent As e On e.entr_idgu=a.guia_idgui
       	\      INNER Join fe_art As aa On aa.idart=e.entr_idar
       	\      INNER Join fe_sucu As s On s.idalma=a.guia_codt,fe_gene  As g
        \      Where  Left(guia_ndoc,1)='T'  And guia_fech Between '<<dfi>>' And '<<dff>>' And guia_moti='T' And guia_acti='A'
		If  nidt > 0 Then
           \ And guia_codt=<<nidt>>
		Endif
	Case This.Motivo = 'O'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,g.Empresa As cliente,guia_deta As Detalle,guia_dcto As Refe,
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'O' As Motivo,guia_codt,
        \      entr_cant As cant,aa.Descri,aa.Unid,s.nomb As Tienda,aa.idart,guia_ptoll As ptoll From fe_guias As a
        \      INNER Join fe_tra As T On T.idtra=a.guia_idtr
        \      INNER Join fe_ent As e On e.entr_idgu=a.guia_idgui
       	\      INNER Join fe_sucu As s On s.idalma=a.guia_codt
        \      INNER Join fe_art As aa On aa.idart=e.entr_idar,fe_gene  As g
        \      Where  Left(guia_ndoc,1)='T'  And guia_fech Between '<<dfi>>' And '<<dff>>' And guia_moti='O' And guia_acti='A'
		If  nidt > 0 Then
          \ And guia_codt=<<nidt>>
		Endif
	Case This.Motivo = 'N'
		\	Select guia_fech As fech, guia_ndoc As Ndoc, c.Razo As cliente, guia_deta As Detalle, '' As Refe,
		\	T.razon As Transportista, guia_idgui As idguia, guia_arch, clie_corr, guia_moti As Motivo, guia_codt,
		\   entr_cant As cant,aa.Descri,aa.Unid,s.nomb As Tienda,aa.idart,guia_ptoll As ptoll From fe_guias As a
		\	INNER Join fe_tra As T On T.idtra = a.guia_idtr
		\	INNER Join fe_rcom As r On r.Idauto = a.guia_idau
		\	INNER Join fe_clie As c On c.idclie = r.Idcliente
	...
	Case This.Motivo = 'I'
        \      Select guia_fech As fech,guia_ndoc As Ndoc,'' As cliente,guia_deta As Detalle,guia_dcto As Refe,
        \      T.razon As Transportista,guia_idgui As idguia,guia_arch,g.correo As clie_corr,'I' As Motivo,guia_codt,
        \      entr_cant As cant,aa.Descri,aa.Unid,s.nomb As Tienda,aa.idart,guia_ptoll As ptoll From fe_guias As a
        \      INNER Join fe_tra As T On T.idtra=a.guia_idtr
        \      INNER Join fe_ent As e On e.entr_idgu=a.guia_idgui
       	\      INNER Join fe_sucu As s On s.idalma=a.guia_codt
	...
	Return 1
	Endfunc
	Function Anula(NAuto, nu)
	lC = "ProAnulaEntregaFisica"
	...
	Return 1
	Endfunc
	Function listarguiasresumen(Ccursor)
	If This.dff - This.dfi > 240 Then
		This.Cmensaje = 'Hasta 240 Días'
		Return 0
	Endif
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	    \      Select guia_fech As fech,guia_ndoc As Ndoc,guia_deta As Detalle,guia_ptoll as ptoll,
		\      IFNULL(T.razon,'') As Transportista,guia_idgui As idguia,guia_moti As Motivo,guia_codt,s.nomb As tienda From
		\      fe_guias As g
		\      Left Join fe_tra As T On T.idtra=g.guia_idtr
		\      INNER Join fe_sucu As s On s.idalma=g.guia_codt
		\      Where guia_fech Between '<<dfi>>' And '<<dff>>' And guia_acti='A'
	If  This.codt > 0 Then
        \ And guia_codt=<<this.codt>>
	Endif
	\ Order By guia_fech,guia_ndoc
	Set Textmerge Off
	Set Textmerge To
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	...
	Return 1
	Endfunc
	Function TieneDespachos(np1)
	Local cur As String
*!*		lC = 'FunVerificaSiEstaAnuladaDespachos'
	Ccursor = 'c_'+Sys(2015)
	goApp.npara1 = np1
	TEXT To lc NOSHOW TEXTMERGE
	SELECT  x.idauto
    FROM `fe_kar` `x`
    JOIN `fe_rcom` `d`         ON `d`.`idauto` = `x`.`idauto`
    JOIN `fe_clie` `e`       ON `e`.`idclie` = `d`.`idcliente`
    LEFT JOIN `fe_ent` `a`      ON `x`.`idkar` = `a`.`entr_idkar`
    LEFT JOIN `fe_guias` `b`     ON `b`.`guia_idgui` = `a`.`entr_idgu`
    WHERE `x`.`acti` = 'A'    AND `d`.`acti` = 'A'  AND `a`.`entr_acti` = 'A'  and x.idauto=<<np1>>
	ENDTEXT
	If This.EJECutaconsulta(lC,Ccursor) < 1  Then
		Return 0
	Endif
	Select (Ccursor)
	If Idauto>0 Then
		This.Cmensaje=' Tiene Guias Remitente Emitidas este Comprobante  '
		Return 0
	Endif
	Return 1
	Endfunc
