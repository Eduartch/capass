Define Class inventarios As OData Of 'd:\capass\database\data.prg'
	...
	Return 1
	Endfunc
	Function consultamvtosresumidospsysr(fi, ff, fii, ccursor)
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	\SELECT q.coda,b.descri,b.unid,si,compras,ventas,stock  FROM (
	\	   SELECT x.coda,sum(si) as si,Sum(compras) As compras,Sum(ventas) As ventas,sum(si)+Sum(compras)-Sum(ventas) As stock from(
	...
	\	   From fe_kar as a
	\	   inner join fe_rcom as b  on  b.idauto=a.idauto
	\	   Where a.tipo='C' and a.acti='A' and b.acti='A' AND fech between '<<fi>>' and '<<ff>>'
	If This.codtienda>0 Then
	 \ and codt=<<this.codtienda>>
	Else
	    \ and tcom<>'T'
	Endif
	\	   Union All
	...
	\	   From fe_kar as c
	\	   inner join fe_rcom as d  on  d.idauto=c.idauto
	\	   Where c.tipo='V' and c.acti='A' and d.acti='A' AND fech between '<<fi>>' and '<<ff>>'
	If This.codtienda>0 Then
	 \ and codt=<<this.codtienda>>
	Else
	    \ and tcom<>'T'
	...
	\	   Where z.acti='A' and y.acti='A' AND  y.fech<'<<fi>>'
	If This.codtienda>0 Then
	 \ and codt=<<this.codtienda>>
	Else
	    \ and tcom<>'T'
	...
	\) as x group by x.coda) as q
	\	   inner join fe_art as b ON b.idart=q.coda
	\ where  si<>0 or compras<>0 or ventas<>0  order by q.coda,b.descri
	Set Textmerge To
	Set Textmerge Off
	If This.ejecutaconsulta(lc, ccursor) < 1 Then
		Return 0
	...
	ccursorx = 'c_' + Sys(2015)
	TEXT To lc Noshow Textmerge
	     select invi_idar as idart FROM fe_inicial WHERE YEAR(invi_fech)=<<YEAR(df)>> and invi_acti='A' limit 1
	ENDTEXT
	If This.ejecutaconsulta(lc, ccursorx) < 1 Then
		Return 0
	Endif
	Select (ccursorx)
	If idart > 0 Then
...
*!*			npreciosingiv=xc.preciosingiv
	...
	Create Cursor tmpk(Fecha D, Tdoc c(2), dcto c(12), Razo c(40), ingr N(12, 2), egre N(12, 2), saldo N(12, 2), Moneda c(12), Precio N(10, 2), Refe c(100)Null, usua c(30)Null, FUsua Datetime Null, usua1 c(10)Null, tipomvto c(20), Idauto N(8), Tipo c(1))
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	...
	Scan All
		...
		If Kardex.Tipo = "C"
			...
			If Isnull(Kardex.proveedor)
				If This.codtienda = Val(Kardex.Ndo2)
					nh = Kardex.codt
				Else
					nh = Val(Kardex.Ndo2)
				Endif
			...
			Case Kardex.Tdoc = "99"
				cm = "Reposiciones"
			Endcase
			Insert Into tmpk(Fecha, Tdoc, dcto, Razo, ingr, saldo, Moneda, Precio, usua, FUsua, usua1, tipomvto, Idauto, Tipo, Refe);
				Values(Kardex.fech, Kardex.Tdoc, Kardex.Ndoc, Crazon, Kardex.cant, calma, Kardex.Cmoneda, Nprecio, Kardex.usua, Kardex.FUsua, Kardex.usua1, cm, Kardex.Idauto, 'C', Kardex.Deta)
		Else
			calma = calma - cant
			egr = egr + cant
			...
			Case Kardex.Tdoc = "99"
				cm = "Reposiciones"
			Endcase
			Insert Into tmpk(Fecha, Tdoc, dcto, Razo, egre, saldo, Moneda, Precio, usua, FUsua, Refe, usua1, tipomvto, Idauto, Tipo, Refe);
				Values(Kardex.fech, Kardex.Tdoc, Kardex.Ndoc, Crazon, Kardex.cant, calma, Kardex.Cmoneda, Nprecio, Kardex.usua, Kardex.FUsua, Kardex.nped, Kardex.usua1, cm, Kardex.Idauto, 'V', Kardex.Deta)
		Endif
	Endscan
	If Sw = 'N'  Then
		...
		If _Screen.oProductos.calcularstockproducto(This.nidart, This.codtienda, 'sinn') < 1 Then
			...
			Return 0
		Endif
		Insert Into tmpk(Razo, saldo)Values("Stock Final", sinn.stock)
	Else
...
*	Strtofile(lC,Addbs(Sys(5)+Sys(2003))+'consulta.txt')
	...
	\   Select D.Ndo2,D.fech,D.Ndoc,D.Tdoc,a.Tipo,D.Mone As Cmoneda,a.cant*a.kar_equi As cant,D.FUsua,IFNULL(g.nomb,'') As usua1,D.codt,
...
*   and tdoc in("01","03","07","08","09","II")
	...
	\   T.nomb As tienda,r.Idauto,k.idart as codigo
	\   From fe_rcom As r
	\   inner Join fe_kar As k On k.Idauto=r.Idauto
	...
	Return 1
	Endfunc
	Function informeAjustespsysg(ccursor)
	f1 = Cfechas(This.dfi)
	f2 = Cfechas(This.dff)
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	...
	\   inner Join fe_sucu As T On T.idalma=r.codt
	\   inner Join fe_usua As u On u.`idusua`=r.`idusua`
	\    Where fech Between '<<f1>>' And '<<f2>>' And r.Acti='A' And k.Acti='A' And Tdoc In('AJ','II')
	If This.codtienda > 0 Then
	\   And r.codt=<<This.codtienda>>
	Endif
	\    Order By fech,Ndoc
   ...
   \Group By c.idart Having (stocki+tingresos+tegresos)>0 ) As b
	...
	m.aumentarigv=''
	Select fe_gene
	If Fsize("gene_exon")>0 Then
		If fe_gene.gene_exon='S' Then
			m.aumentarigv='S'
		Endif
	Endif
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	\	select b.fech,b.ndoc,b.tdoc,a.tipo,a.cant,
	If m.aumentarigv='S' Then
	   \ ROUND(a.prec*b.vigv,2) as prec,
	Else
	   \ ROUND(a.prec,2) as prec,
	Endif
	\   b.mone,b.idcliente,c.razo as cliente,b.idprov,e.razo as proveedor,
	\	b.dolar as dola,b.vigv as igv,b.idauto,a.idkar,a.idart
	\	from fe_rcom as b
...
*
	...
	\From fe_rcom As r
	\inner Join fe_kar As k On k.`Idauto`=r.`Idauto`
	\Where fech Between '<<f1>>' And '<<f2>>' And (Idcliente>0 Or idprov>0) And k.Acti='A' And r.`Acti`='A'
	...
	If This.nidart < 1 Then
		...
		Return 0
	Endif
	oprod = Newobject("productosmoviza","d:\capass\modelos\productosmoviza.prg")
	dfechaI = This.dfi
	dfechaf = This.dff
	ccoda = This.nidart
	fi = Cfechas(This.dfi)
	...
	Endscan
	If Sw = 'N'  Then
		If oprod.calcularstockproductogmoviza(ccoda, This.codtienda, 'sinn') < 1 Then
...
*MESSAGEBOX(lc)
	...
	If Sw = 'N'  Then
		...
		If oprod.calcularstockproductopsysr(Trim(This.ccodart), This.codtienda, 'sinn') < 1 Then
			This.Cmensaje = oprod.Cmensaje
			Return 0
		Endif
	...
	Return 1
	Endfunc
	Function consultarkardexgrifos(ccursor)
	dfechaI = Cfechas(This.dfi)
	dfechaf = Cfechas(This.dff)
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	   ...
	   \inner Join fe_usua As F On(F.idusua=D.idusua)
	   \Left Join fe_usua As g On (g.idusua=D.idusua1)
	   \Where a.idart=<<This.nidart>> And D.Acti<>'I' And D.fech Between '<<dfechai>>' And  '<<dfechaf>>' And a.Acti<>'I' And D.rcom_ccaj<>'C'
	If This.codtienda > 0 Then
	     \ And a.alma=<<This.codtienda>>
	Endif
	If This.agrupadopordia = 'S' Then
	   \Order By D.fech,a.Tipo,D.Tdoc,D.Ndoc
	Else
	...
	Return 1
	Endfunc
	Function consultarmvtosinventariocantidades(ccursor)
	ff=Cfechas(This.dff)
	fi=Cfechas(This.dfi)
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
	\select  a.fech,a.ndoc,a.tdoc,b.tipo,b.cant,b.Prec,a.vigv as igv,a.idcliente as codc,c.razo As cliente,a.idprov as codp,
	\d.razo As proveedor,a.ndo2,a.idauto,b.idart From fe_kar  as b
	\INNER JOIN fe_rcom as a ON a.idauto=b.idauto
	\Left Join fe_prov as d  On d.idprov=a.idprov
	\Left Join fe_clie as c On c.idclie=a.idcliente
	\Where a.fech  between '<<fi>>' and  '<<ff>>' and a.acti='A' and b.acti='A'
	If This.codtienda>0 Then
	 \ and a.codt=<<this.codtienda>>
	Else
	\ and  tcom<>'T'
	Endif
	Set Textmerge To
	...
	Return 1
	Endfunc
	Function listarTraspaso(nid,ccursor)
	TEXT to lc NOSHOW TEXTMERGE PRETEXT 7
       SELECT a.descri,a.unid,b.cant,y.nomb as salida,x.nomb as ingresa,c.ndo2,f.nomb as usua,c.fusua,c.ndoc,a.idart as coda,
       c.idauto,c.codt,b.prec,c.fech FROM fe_rcom as c
       inner join fe_kar as b on b.idauto=c.idauto
       inner join fe_usua as f on f.idusua=c.idusua
       inner join fe_art as a on a.idart=b.idart
       inner join fe_sucu as x on x.idalma=c.ndo2
       inner join fe_sucu as y on y.idalma=c.codt
       where c.idauto=<<nid>> AND b.acti='A' order by idkar
	ENDTEXT
	If This.ejecutaconsulta(lc,ccursor)<1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function recibirtraspaso(nid,nidtda,nidtda1)
	Sw=1
	If !Pemstatus(goApp,'productoscp',5) Then
		AddProperty(gopp,'productoscp','')
	Endif
	okar = Newobject("regkardex","d:\capass\modelos\regkardex.prg")
	If This.IniciaTransaccion()<1 Then
		Return  0
	Endif
	okar.nidauto=m.nid
	...
	Scan All
		...
		okar.ncant = ed.cant
		okar.nprec = ed.Prec
		okar.nidtda = m.nidtda
		okar.nidtda1 = m.nidtda1
		okar.Ctipo = 'C'
		okar.ncosto = ed.Prec
		okar.Cdetalle = 'Traspasos de Ingresos Con Verificacion'
		If okar.IngresaDtraspasos() < 1 Then
			This.Cmensaje = okar.Cmensaje
			Sw = 0
			Exit
		Endif
		_Screen.oProductos.ncoda= ed.Coda
		_Screen.oProductos.ncant=ed.cant
		If _Screen.oProductos.ActualizaStock(ed.Coda, nidtda, ed.cant, 'C') <1  Then
			Sw = 0
			This.Cmensaje =_Screen.oProductos.Cmensaje
			Exit
		Endif
		If goApp.productoscp='S' Then
			If _Screen.oProductos.ActualizastockTransito('V') < 1 Then
				Sw = 0
				This.Cmensaje = _Screen.oProductos.Cmensaje
				Exit
			Endif
	...
	Return 1
	Endfunc
	Function buscarTrapasoporid(cTdoc,cndoc,nid,ccursor)
	If !Pemstatus(goApp,'productoscp',5) Then
		AddProperty(goap,'productoscp','')
	Endif
	Set Textmerge On
	Set Textmerge To Memvar lc Noshow Textmerge
    \ SELECT a.alma,a.idkar,b.descri,b.unid,a.idart,a.cant,a.prec,c.impo,c.valor,c.igv,ndo2,
    \ c.fech,a.tipo,c.tdoc,c.ndoc,c.mone,c.idauto,c.rcom_reci,b.prod_cod1
	If goApp.productoscp='S' Then
       \,b.uno,b.dos,b.tre,b.cua,b.cero,b.cin,b.sei,b.sie,b.och,b.nue,b.die,b.onc,b.doce,b.caracteris,b.prod_tran
	Else
       \,b.uno,b.dos,b.tre,b.cua,b.cin
	Endif
    \FROM fe_kar as a
    \ inner JOIN  fe_art as b ON (b.idart=a.idart)
    \ inner join fe_rcom as c ON(c.idauto=a.idauto)
	...
	If m.nid >0 Then
       \ c.idauto=<<nid>>
	Else
       \  c.ndoc='<<cndoc>>' and c.tdoc='<<ctdoc>>'
	Endif
       \ and a.tipo='V' AND c.ACTI<>'I' and a.acti='A' order by idkar
	Set Textmerge Off
	Set Textmerge To
	If This.ejecutaconsulta(lc,ccursor)<1 Then
		Return 0
