Define Class lmayor As OData Of 'd:\capass\database\data.prg'
	...
	dfii = Ctod('01/01/' + Trim(Str(Na)))
	ofechas = dfianddff(This.nmes, This.Na)
	fi = cfechas(ofechas.dfi)
	ff = cfechas(ofechas.dff)
	dfi=cfechas(m.dfii)
	If This.nmes = 1 Then
		Set Textmerge On
		Set Textmerge To  Memvar lC  Noshow
		   ...
		   \Where a.ldia_acti='A' And ldia_fech = '<<fi>>' And ldia_tran<>'T'  And ldia_inic='I'
	...
	Else
		...
		\	From fe_ldiario As a
		\   inner Join fe_plan As b On b.idcta=a.ldia_idcta
		\	Where a.ldia_acti='A' And ldia_fech >= '<<dfi>>' and ldia_fech< '<<fi>>' And ldia_tran<>'T'
		If This.cmultiempresa = 'S' Then
		   \ And ldia_codt=<<This.ncodt>>
		Endif
		   \ Group By a.ldia_idcta) As z
