Define Class Ventas As OData Of 'd:\capass\database\data.prg'
	...
	If  goapp.vtascondetraccion = 'S' Then
		If  Len(Alltrim(This.coddetraccion)) > 0 Then
			If Len(Alltrim(_Screen.oimportar.codigodetraccion(This.coddetraccion))) = 0 Then
				This.Cmensaje = "Codigo de Detracción no Valido"
				Return .F.
			Endif
		Endif
	Endif
	...
	Case This.PermiteIngresox() < 1
		...
		Return .F.
*!*		Case (fe_gene.fech - This.Fecha) > 3 And (This.Tdoc = '01' Or This.Tdoc = '03' Or This.Tdoc = '07' Or This.Tdoc = '08')
*!*			This.Cmensaje = "No es posible Grabar con esta Fecha. Tiene Como Máximo 3 días de Atraso"
*!*			Return .F.
...
*!*			Return .F.
	...
	Case This.Monto = 0
		This.Cmensaje = "Ingrese Cantidad y Precio"
		Return .F.
	Case This.Serie = "0000" Or Val(This.numero) = 0 Or Len(Alltrim(This.Serie)) <> 4 Or Len(Alltrim(This.numero)) < 8
		This.Cmensaje = "Ingrese Un Número de Documento Válido"
		Return .F.
	Case Empty(This.Almacen)
		This.Cmensaje = "Seleccione Un Almacen"
	...
	If This.chkdetraccion = 1 And Len(Alltrim(This.coddetraccion)) = 3 Then
		If Len(Alltrim(_Screen.oimportar.codigodetraccion(This.coddetraccion))) = 0 Then
			This.Cmensaje = "Codigo de Detracción no Valido"
			Return .F.
		Endif
	...
	If Len(Alltrim(This.temporal)) > 0 And VerificaAlias(This.temporal) = 1
		...
		If Fsize("unidad") > 0 Then
			Select Unidad From (Calias) Where Nitem > 0 And Len(Alltrim(Unidad)) < 2 Into Cursor tvalidar
		Else
			Select Unid From (Calias) Where Nitem > 0 And Len(Alltrim(Unid)) < 2 Into Cursor tvalidar
		Endif
		If _Tally > 0 Then
	...
	Text To lC Noshow Textmerge
	  SELECT  `c`.`rcom_exon` AS `rcom_exon`,  `c`.`idusua`    AS `idusua`,  `a`.`kar_comi`  AS `comi`,
	  `a`.`codv`      AS `codv`,  `a`.`idauto`    AS `idauto`,  `c`.`codt`      AS `alma`,
	  `a`.`kar_idco`  AS `idcosto`,  `a`.`idkar`     AS `idkar`,  `a`.`idart`     AS `idart`,
	  `a`.`cant`      AS `cant`,  `a`.`prec`      AS `prec`,  `c`.`valor`     AS `valor`,
	  `c`.`igv`       AS `igv`,  `c`.`impo`      AS `impo`,  `c`.`fech`      AS `fech`,
	  `c`.`fecr`      AS `fecr`,  `c`.`form`      AS `form`,  `c`.`deta`      AS `deta`,
	  `c`.`exon`      AS `exon`,  `c`.`ndo2`      AS `ndo2`,  `c`.`rcom_entr` AS `rcom_entr`,
	  `c`.`idcliente` AS `idclie`,  `d`.`razo`      AS `razo`,  `d`.`nruc`      AS `nruc`,
	  `d`.`dire`      AS `dire`,  `d`.`ciud`      AS `ciud`,  `d`.`ndni`      AS `ndni`,
	  `a`.`tipo`      AS `tipo`,  `c`.`tdoc`      AS `tdoc`,  `c`.`ndoc`      AS `ndoc`,
	  `c`.`dolar`     AS `dola`,  `c`.`mone`      AS `mone`,  `b`.`descri`    AS `descri`,
	  `b`.`unid`      AS `unid`,  `b`.`premay`    AS `pre1`,  `b`.`peso`      AS `peso`,  `b`.`premen`    AS `pre2`,
	  `c`.`vigv`      AS `vigv`,  `c`.`idcliente` AS `idcliente`,  `c`.`codt`      AS `codt`,    `b`.`uno`       AS `uno`,  `b`.`dos`       AS `dos`,  `c`.`fusua`     AS `fusua`,  `p`.`nomv`      AS `Vendedor`,  `q`.`nomb`      AS `Usuario`,
	  ROUND(a.cant*a.prec,2) AS importe,IFNULL(s.seriep,"") AS serieproducto,IFNULL(idseriep,0) AS idseriep
	  FROM `fe_rcom` `c`
      ...
      WHERE g.dser_acti='A' AND rser_acti='A') AS s ON s.dser_idka=a.idkar
      WHERE  c.idauto=<<np1>>
	Endtext
...
*!*	       where g.dser_acti='A' and rser_acti='A') as s ON s.dser_idka=a.idkar WHERE a.idauto=<<np1>>
	...
	Set Textmerge On
	Set Textmerge To Memvar  lC Noshow Textmerge
	\   Select  c.Razo,a.Descri,
	If goapp.Proyecto == 'psysg' Then
	  \k.kar_unid As Unid,
	Else
      \a.Unid,
	Endif
    \   k.cant,k.Prec,k.idart,k.alma,r.idcliente As idclie,r.Idauto,rcom_idtr,
	\   r.fech,r.valor,r.igv,r.Impo,r.Mone,u.nomb As Usuario,r.FUsua,Ndoc,idkar,c.nruc,c.ndni As dni,r.Tdoc,v.nomv As Vendedor,r.Form As Forma From fe_rcom As r
	\   inner Join fe_clie As c On c.idclie=r.idcliente
	\   inner Join fe_kar As k On k.Idauto=r.Idauto
	\   inner Join fe_art As a On a.idart=k.idart
	\   inner Join fe_usua As u  On u.idusua=r.idusua
	...
	Case This.tdocref = '01' And  !'FD' $ Left(This.Serie, 2) And This.Tdoc = '08'
		This.Cmensaje = "Número del  Documento NO Válido"
		Return 0
	Case Left(Alltrim(This.Serie), 2) <> 'FN'  And Left(Alltrim(This.Serie), 2) <> 'BC' And  ;
			Left(Alltrim(This.Serie), 2) <> 'FD' And Left(Alltrim(This.Serie), 2) <> 'BD'
		This.Cmensaje = "Serie del Documento no Válido"
		Return 0
	Case This.Codigo < 1
	...
	Case Left(This.Tiponotacredito, 2) = '13' And This.montonotacredito13 = 0
		...
		Return 0
	Case onc.verificancventas(This.nidautoref) < 1
	...
	Text To lC Noshow Textmerge
       SELECT  idauto FROM fe_rcom WHERE ndoc='<<cdcto>>' AND tdoc='<<ctdoc>>' and acti<>'I' AND idcliente>0 limit 1
	Endtext
	Ccursor = 'c_' + Alltrim(Sys(2015))
	If This.EJECutaconsulta (lC, Ccursor) < 1 Then
		Return 0
	Endif
	Select (Ccursor)
...
*:Global cur
	...
	Return 1
	Endfunc
	Function ventasporclientepsystr(Ccursor)
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
    \Select fech,fecr,Tdoc,Ndoc,Ndo2,Mone,valor,igv,Impo,dolar As dola,Form,
	\  cant,Round(k.Prec,2) As Prec,Round(k.cant*k.Prec,2) As Importe,dsnc,dsnd,gast,Descri,Unid,u.nomb As Usuario,FUsua,r.Idauto,r.Deta,r.alma As codproy,tt.nomb As tienda From
	\  fe_rcom As r
	\  inner Join (Select idart,cant,Prec,dsnc,dsnd,gast,Idauto From fe_kar Where Acti='A') As k  On k.Idauto=r.Idauto
	\  inner Join fe_art As b On b.idart=k.idart
	\  inner Join fe_usua As u On u.idusua=r.idusua
	\  inner Join fe_clie As cc On cc.idclie=r.idcliente
	\  inner Join fe_sucu As tt On tt.idalma=r.codt
	\  Where r.fech Between '<<f1>>' And '<<f2>>' And r.Acti='A'
	If This.Codigo > 0 Then
	\And r.idcliente=<<This.Codigo>>
	Endif
	If This.codt > 0 Then
	    \ And r.codt=<<This.codt>>
	Endif
	If Left(goapp.tipousuario, 1) = 'V'  And goapp.productoscp = 'S' Then
	   \And r.idusua=<<goapp.nidusua>>
	Endif
	\  Union All
	\  Select fech,fecr,Tdoc,Ndoc,Ndo2,Mone,valor,igv,Impo,dolar As dola,Form,
	\  cant,Round(k.Prec,2) As Prec,Round(k.cant*k.Prec,2) As Importe,dsnc,dsnd,gast,k.Descri,Unid,u.nomb As Usuario,FUsua,r.Idauto,r.Deta,r.alma As codproy,tt.nomb As tienda From
	\  fe_rcom As r
	\  inner Join (Select detv_item As idart,detv_desc As Descri,'' As Unid,detv_cant As cant,detv_prec As Prec,1 As dsnc,
	\  1 As dsnd,1 As gast,detv_idau
	\  From fe_detallevta Where detv_acti='A') As k   On k.detv_idau=r.Idauto
	\  inner Join fe_usua As u On u.idusua=r.idusua
	\  inner Join fe_clie As cc On cc.idclie=r.idcliente
	\  inner Join fe_sucu As tt On tt.idalma=r.codt
	\  Where r.fech Between '<<f1>>' And '<<f2>>' And r.Acti='A'
	...
	Set Textmerge To Memvar lC Noshow
	   ...
	   \inner Join fe_clie  As b On(b.idclie=a.idcliente)
	   \Where fech Between '<<f1>>' And '<<f2>>'   And Tdoc In ('01','03','07','08') And Acti='A'
...
*!*		STRTOFILE(lc,ADDBS(SYS(5)+SYS(2003))+'consulta.txt')
	...
	If goapp.Proyecto = 'psysn' Then
	 \ a.kar_comi*((a.cant*a.Prec)/e.vigv) As comi,
	Else
	\a.kar_comi As comi,
	Endif
	\a.Idauto, e.Tdoc, e.Ndoc, e.fech, b.idart, a.cant, a.Prec, Round(a.cant * a.Prec, 2) As timporte,
	\e.Mone, a.alma, a.idart, b.idmar, c.nomv As nomb, e.Form,
	\e.vigv As igv, a.Codv, Cast(e.dolar As Decimal(6,4)) As dola, b.Descri, d.Razo, m.dmar As marca, d.nruc, d.ndni, b.prod_cod1,s.nomb As tienda,e.codt
	If goapp.Proyecto = 'psysg' Then
	\,a.kar_unid As Unid
	Else
	\,b.Unid
	...
	Endif
	\Order By c.nomv,a.Codv,a.Idauto,e.Mone
	Set Textmerge Off
	Set Textmerge To
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	...
	\Where rcre_acti='A' And Acti='A' And fech Between '<<f1>>' And '<<f2>>' Group By rcre_idau)  As p On p.rcre_idau=a.Idauto
	...
	Create Cursor registro(Form c(1) Null, fech d Null, fvto d, Tdoc c(2) Null, Serie c(4), Ndoc c(8),  nruc c(11)Null, ndni c(8), Razo c(100)Null, valorg N(14, 2), Exon N(12, 2), ;
		  ...
		  Auto N(15), T N(1), fecr d Null, pimpo N(8, 2), inafecta N(12, 2))
	x = 1
	If This.Listarnotascreditoydebito("xnotas") < 1 Then
		Return 0
	Endif
	...
	Create Cursor registro(Form c(2) Null, fecr d Null, fech d Null, Tdoc c(2) Null, Serie c(4), Ndoc c(8), nruc c(11)Null, Razo c(120)Null, valorg N(14, 2), Exon N(12, 2), ;
		  igvg N(10, 2), Importe N(14, 2), pimpo N(8, 2), grati N(10, 2), Detalle c(50), Usuario c(30), FUsua T, Mensaje c(120), dscto N(10, 2), ndni c(8),  dola N(5, 3), Mone c(1), Codigo N(5), fechn d, tref c(2), Refe c(12), Fevto d, Auto N(15),  ;
	...
	Text To lC Noshow Textmerge Pretext 7
		...
		k.detv_prec as costo,CAST(0 as decimal(2)) as codv from fe_rcom r
	...
	Return 1
	Endfunc
	Function buscarxidpsysg(nidauto, Ccursor)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Text To lC Noshow Textmerge
	  SELECT `c`.`rcom_icbper` AS `rcom_icbper`,  `a`.`kar_icbper`  AS `kar_icbper`,  `c`.`rcom_mens`   AS `rcom_mens`,  `c`.`rcom_idtr`   AS `rcom_idtr`,
	  `a`.`codv`        AS `codv`,  `a`.`idauto`      AS `idauto`,  `a`.`kar_posi`    AS `kar_posi`,  `a`.`kar_equi`    AS `kar_equi`,  `a`.`kar_epta`    AS `kar_epta`,  `a`.`kar_comi`    AS `kar_comi`,
	  `c`.`codt`        AS `alma`,  `a`.`kar_idco`    AS `idcosto`,  `a`.`idkar`       AS `idkar`,  `a`.`idart`,  `a`.`cant`        AS `cant`,  `a`.`prec`        AS `prec`,
	  `c`.`valor`       AS `valor`,  `c`.`igv`         AS `igv`,  `c`.`impo`        AS `impo`,  `c`.`fech`        AS `fech`,  `c`.`fecr`        AS `fecr`,  `c`.`form`        AS `form`,
	  `c`.`deta`        AS `deta`,  `c`.`exon`        AS `exon`,  `c`.`ndo2`        AS `ndo2`,  `c`.`rcom_entr`   AS `rcom_entr`,  `c`.`idcliente`   AS `idclie`,  `d`.`razo`        AS `razo`,
	  `d`.`nruc`        AS `nruc`,  `d`.`dire`        AS `dire`,  `d`.`ciud`        AS `ciud`,  `d`.`ndni`        AS `ndni`,  `a`.`tipo`        AS `tipo`,  `c`.`tdoc`        AS `tdoc`,
	  `c`.`ndoc`        AS `ndoc`,  `c`.`dolar`       AS `dolar`,  `c`.`mone`        AS `mone`,  `b`.`descri`      AS `descri`,  IFNULL(`x`.`idcaja`,0) AS `idcaja`,
	  `a`.`kar_unid`        AS `unid`,  `b`.`premay`      AS `pre1`,  `b`.`peso`        AS `peso`,  `b`.`premen`      AS `pre2`,  IFNULL(`z`.`vend_idrv`,0) AS `nidrv`,
	  `c`.`vigv`        AS `vigv`,  `a`.`dsnc`        AS `dsnc`,  `a`.`dsnd`        AS `dsnd`,  `a`.`gast`        AS `gast`,  `c`.`idcliente`   AS `idclie`,   `b`.`pre3`        AS `pre3`,
	  `b`.`cost`        AS `costo`,  `b`.`uno`         AS `uno`,  `b`.`dos`         AS `dos`,  (`b`.`uno` + `b`.`dos`) AS `TAlma`,  `c`.`fusua`       AS `fusua`,  `p`.`nomv`        AS `Vendedor`,
	  `q`.`nomb`        AS `Usuario`
	  FROM `fe_art` `b`
	...
	WHERE c.idauto=<<nidauto>>   AND `c`.`acti` <> 'I'    AND `a`.`acti` <> 'I'
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	Text To lC Noshow Textmerge
	  SELECT   `a`.`kar_comi`  ,  `a`.`codv`   ,  `a`.`idauto`  ,  `c`.`codt`      AS `alma`,
	  `a`.`kar_idco` as idcosto  ,  `a`.`idkar`,  `a`.`idart` ,  `a`.`cant`      AS `cant`,
	  `a`.`prec`     ,  `c`.`valor`     AS `valor`,  `c`.`igv`       AS `igv`,  `c`.`impo`      AS `impo`,
	  `c`.`fech`,  `c`.`fecr`      AS `fecr`,  `c`.`rcom_dsct` AS `rcom_dsct`,  `c`.`rcom_mens` AS `rcom_mens`,
	  `c`.`form` ,  `c`.`deta`      AS `deta`,  `c`.`exon`      AS `exon`,  `c`.`ndo2`      AS `ndo2`,
	  `c`.`rcom_entr` ,  `c`.`idcliente` AS `idclie`,  `d`.`razo`      AS `razo`,  `d`.`nruc`      AS `nruc`,
	  `d`.`dire`      ,  `d`.`ciud`      AS `ciud`,  `d`.`ndni`      AS `ndni`,  `a`.`tipo`      AS `tipo`,
	  `c`.`tdoc`      ,  `c`.`ndoc`      AS `ndoc`,  `c`.`dolar`     AS `dolar`,  `c`.`mone`      AS `mone`,
	  `b`.`descri`    ,  IFNULL(`x`.`idcaja`,0) AS `idcaja`,  `b`.`unid`      AS `unid`,  `b`.`premay`    AS `pre1`,
	  `b`.`peso`      ,  `b`.`premen`    AS `pre2`,  IFNULL(`z`.`vend_idrv`,0) AS `nidrv`,
	  `c`.`vigv`      ,  `a`.`dsnc`      AS `dsnc`,  `a`.`dsnd`      AS `dsnd`,  `a`.`gast`      AS `gast`,
	  `c`.`idcliente` AS `idclie`,  `c`.`codt`      AS `codt`,  IFNULL(b.pre3,0) AS pre3,  `b`.`cost`      AS `costo`,
	  `b`.`uno`      ,  `b`.`dos`       AS `dos`,  b.tre,  (((((`b`.`uno` + `b`.`dos`) + `b`.`sei`) + `b`.`cin`) + `b`.`cua`) + `b`.`nue`) AS `TAlma`,
	  `b`.`sei`       AS `sei`,  `b`.`cua`       AS `cua`,  `b`.`cin`       AS `cin`,  b.sie,b.och,
	  `b`.`nue`       ,  b.die,  `a`.`kar_codi`  AS `kar_codi`,  `c`.`fusua`     AS `fusua`,IFNULL(p.fevto,c.fech) AS fvto,
	...
	Text To lC Noshow Textmerge
	  ...
	  a.kar_idco    AS idcosto,  a.idkar       AS idkar,  a.idart,  a.cant,
	  a.prec        AS prec,  c.valor,  c.igv,  c.impo,c.fech, c.fecr,  c.form, c.deta,
	  ...
	  d.ndni        AS ndni,  a.tipo        AS tipo,  c.tdoc        AS tdoc,  c.ndoc,
	  c.dolar       AS dolar,  c.mone        AS mone,  b.descri      AS descri,  0                 AS idcaja,
	  b.unid        AS unid,  b.premay      AS pre1,  b.peso        AS peso,  b.premen      AS pre2,
	  CAST(0 AS DECIMAL(4,2)) AS nidrv,  c.vigv,  a.dsnc ,  a.dsnd,
	  a.gast        AS gast,  c.codt,  b.pre3        AS pre3,if(tmon='S',b.prec*g.igv,b.prec*g.dola*g.igv) as costo,
	  b.cost ,  b.uno         AS uno,  b.dos         AS dos,  b.uno + b.dos AS TAlma,
	  c.fusua,  p.nomv        AS Vendedor,  q.nomb        AS Usuario,  c.rcom_icbper,  a.kar_icbper,
	  c.rcom_mens,rcom_mdet,rcom_detr,ifnull(p.fevto,c.fech) as fvto,clie_rete
	  FROM fe_rcom as c
	  JOIN fe_kar a   on a.idauto=c.idauto
	  JOIN fe_art b   ON b.idart = a.idart
	  JOIN fe_clie d  ON c.idcliente = d.idclie
	  ...
	  left join (select rcre_idau,min(c.fevto) as fevto from fe_rcred as r
      inner join fe_cred as c on c.cred_idrc=r.rcre_idrc
      where rcre_acti='A' and acti='A' and rcre_idau=<<nidauto>> group by rcre_idau) as p on p.rcre_idau=a.idauto,fe_gene AS g
	...
	Return 1
	Endfunc
	Function buscarvcentaxidpsysn(nidauto, Ccursor)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Text To lC Noshow Textmerge
	SELECT  `a`.`kar_cost`  AS `kar_cost`,  `c`.`idusua`    AS `idusua`,  `a`.`kar_comi`  AS `kar_comi`,  `a`.`codv`      AS `codv`,  `a`.`idauto`    AS `idauto`,
	  `a`.`alma`      AS `alma`,  `a`.`kar_idco`  AS `idcosto`,  `a`.`idkar`     AS `idkar`,  `a`.`idart` ,  `a`.`cant`      AS `cant`,
	  `a`.`prec`      AS `prec`,  `c`.`valor`     AS `valor`,  `c`.`igv`       AS `igv`,  `c`.`impo`      AS `impo`,  `c`.`fech`      AS `fech`,
	  `c`.`fecr`      AS `fecr`,  `c`.`form`      AS `form`,  `c`.`deta`      AS `deta`,  `c`.`exon`      AS `exon`,  `c`.`ndo2`      AS `ndo2`,
	  `c`.`rcom_entr` AS `rcom_entr`,  `c`.`idcliente` AS `idclie`,  `d`.`razo`      AS `razo`,  `d`.`nruc`      AS `nruc`,
	  `d`.`dire`      AS `dire`,  `d`.`ciud`      AS `ciud`,  `d`.`ndni`      AS `ndni`,  `a`.`tipo`      AS `tipo`,  `c`.`tdoc`      AS `tdoc`,
	  ...
	  `c`.`vigv`      AS `vigv`,  `a`.`dsnc`      AS `dsnc`,  `a`.`dsnd`      AS `dsnd`,  `a`.`gast`      AS `gast`,  `c`.`idcliente` AS `idcliente`,
	  `c`.`codt`      AS `codt`,  `b`.`pre3`      AS `pre3`,  `b`.`cost`      AS `costo`,  `b`.`uno`       AS `uno`,  `b`.`dos`       AS `dos`,
	  `b`.`tre`       AS `tre`,  `b`.`cua`       AS `cua`,  (`b`.`uno` + `b`.`dos`) AS `TAlma`,  `c`.`fusua`     AS `fusua`,  `p`.`nomv`      AS `Vendedor`,
	  `q`.`nomb`      AS `Usuario`,  `c`.`rcom_idtr` AS `rcom_idtr`,  `c`.`rcom_tipo` AS `rcom_tipo`,  `c`.`rcom_mens` AS `rcom_mens`
	FROM `fe_rcom` `c`
	    JOIN `fe_kar` `a`    ON   `a`.`idauto` = `c`.`idauto`
	    ...
	    JOIN `fe_vend` `p`   ON   `p`.`idven` = `a`.`codv`
	    JOIN `fe_usua` `q`    ON   `q`.`idusua` = `c`.`idusua`
	    JOIN `fe_rvendedor` `z`   ON   `z`.`vend_idau` = `c`.`idauto`
	WHERE   c.idauto=<<niDAUTO>> AND  `a`.`acti` <> 'I'  order by idkar
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	If NAuto < 1 Then
		...
		Return 0
	Endif
	If IngresaDatosLCajaEFectivo12(This.Fecha, "", This.clienteseleccionado, This.cta3, This.Monto, 0, 'S', fe_gene.dola, m.nidcajero, This.Codigo, NAuto, Left(This.formaPago, 1), This.Serie + This.numero, This.Tdoc, goapp.tienda) < 1 Then
		This.DEshacerCambios()
		Return 0
	Endif
	...
	Endif
	If goapp.vtascondetraccion = 'S' Then
		If This.ActualizaResumenDctovtascondetraccion1() < 1 Then
			This.DEshacerCambios()
			Return 0
		Endif
	Else
	...
	Select ddd
	Scan All
		If Len(Alltrim(Cdetalle)) < 400 Then
			If x = 0 Then
...
*!*		wait WINDOW 'hola'
*!*		wait WINDOW this.serie
*!*		wait WINDOW this.numero
*!*		wait WINDOW this.tdoc
	If Len(Alltrim(This.Serie)) = 0  Then
		This.Cmensaje = "Ingrese Serie"
		Return 0
	Endif
	If  Len(Alltrim(This.numero)) = 0
		This.Cmensaje = "Ingrese Número"
		Return 0
	Endif
	If Len(Alltrim(This.Tdoc)) = 0 Then
		This.Cmensaje = "Ingrese Tipo de Documento "
		Return 0
	Endif
	cndoc = This.Serie + This.numero
	Ccursor = 'c_' + Sys(2015)
	...
	Else
		If This.Idanticipo > 0 Then
			NAuto = This.registrarxserviciosconanticipo1()
		Else
			NAuto = IngresaDocumentoElectronico(This.Tdoc, Left(This.formaPago, 1), This.Serie + This.numero, This.Fecha, This.Detalle, This.valor, This.igv, This.Monto, "", ;
				  Left(This.Moneda, 1), This.ndolar, This.vigv, 'S', This.Codigo, "V", goapp.nidusua, This.codt, This.cta1, This.cta2, This.cta3, This.Vendedor, 0, This.exonerado, 0)
		Endif
...
*!*			Endif
	...
	Else
		If This.Idanticipo > 0 Then
			If This.ActualizarResumenDctovtasconAnticipo() < 1 Then
				This.DEshacerCambios()
				Return 0
			Endif
		Else
	...
	Set Textmerge To Memvar lC Noshow Textmerge
	    ...
	    \inner Join  (Select Idauto,Codv From fe_kar As a Where alma>0 And Acti='A' And tipo='V' Group By a.Idauto Order By a.Idauto ) As k On k.Idauto=a.Idauto
	    \inner Join fe_vend As v On v.idven=k.Codv
    ...
    \ d.Descri,
	Endif
	\d.Unid,e.cant,e.Prec,a.Mone,F.nomb As Usuario,e.cant*e.Prec  As Impo,a.Form,valor,igv,Impo As Importe,e.idart As Codigo,s.nomb As Almacen
	\From fe_rcom As a
	...
	If Len(Alltrim(This.Tdoc)) > 0 Then
	   \ And a.Tdoc='<<this.tdoc>>'
	Endif
	If fe_gene.nruc = '20601140625' And Left(goapp.tipousuario, 1) <> 'A' Then
	...
	If This.AgrupadaGanancia = 'S' Then
	    \Select Ndoc,fech,cliente,Vendedor,Importe,If(Utilidad<>0,(Sum(Utilidad)*100)/Sum(costototal),Cast(0 As Decimal(10,2))) As porcentaje,Sum(Utilidad) As Utilidad,tienda,Idauto From
	    ...
	    \cc.Razo As cliente,v.`nomv` As Vendedor,T.nomb As tienda,c.Idauto,Ndoc,fech,If(c.Mone='S',Impo,Impo*c.dolar) As Importe,m.dmar As marca
	    \From fe_rcom As c
	...
	Else
	    ...
	    \cc.Razo As cliente,v.`nomv` As Vendedor,Ndoc,T.nomb As tienda,
		If  goapp.productoscp = 'S' Then
	       \  ifnull(uu.nomb,'') As responsable,
		Else
	      \'' As responsable,
		Endif
	    \fech,c.Idauto,k.idart As coda
	    \ From fe_rcom As c
		\inner Join fe_kar As k On k.Idauto=c.Idauto
		\inner Join fe_art As b On b.idart=k.idart
		\inner Join fe_mar As m On m.idmar=b.idmar
	    ...
	    \inner Join fe_sucu As T On T.idalma=c.codt
		If goapp.productoscp = 'S' Then
		  \ Left Join fe_usua As uu On uu.idusua=b.prod_idur
		Endif
	    \Where k.Acti='A' And c.Acti='A' And c.fech Between  '<<dfi>>' And '<<dff>>'   And c.tcom<>'T'
		If This.nmarca > 0 Then
	    \ And b.idmar=<<This.nmarca>>
	...
	\a.igv,a.Impo,rcom_mens,rcom_fecd,u.nomb,FUsua,rcom_hash,a.Tdoc,a.Ndoc,a.Idauto,rcom_arch,b.clie_corr,tcom,b.fono,b.celu,Ndo2,
	\Concat(Trim(b.Dire),' ',Trim(b.ciud)) As Dire,rcom_otro,b.ndni,nruc As Ruc,Form,dolar,a.codt,a.tipom,a.idusua,Deta,a.vigv,ifnull(p.Fevto,a.fech) As Fevto,vv.nomv As Vendedor
	...
	\    Left Join (Select rcre_idau,Min(c.Fevto) As Fevto From fe_rcred As r inner Join fe_cred As c On c.cred_idrc=r.rcre_idrc
    \    Where rcre_acti='A' And Acti='A'  And c.fech Between '<<f1>>' And '<<f2>>' Group By rcre_idau) As p On p.rcre_idau=a.Idauto
    \    Left Join (Select Idauto,Codv From fe_kar Where Acti='A' And Codv>0 Group By Idauto,Codv) As rv On rv.Idauto=a.Idauto
    \    Left Join fe_vend As vv On vv.idven=rv.Codv
	...
	Endif
	If Len(Alltrim(This.formaPago)) > 0 Then
	     \ And a.Form='<<LEFT(this.formapago,1)>>'
	Endif
...
*MESSAGEBOX(lc)
	...
	Scan All
		Text To lC Noshow  Textmerge
	     UPDATE fe_kar SET prec=<<tmpp.prec>>,codv=<<this.vendedor>>,idauto=<<this.Idauto>>  where idkar=<<tmpp.nreg>>
		Endtext
		If This.Ejecutarsql(lC) < 1 Then
			Sw = 0
			Exit
	...
	Select tmpv
	Replace All cletras With This.cletras, hash With This.hash, Archivo With This.ArchivoXml, fech With This.Fecha, Tdoc With This.Tdoc In tmpv
	Select tmpv
	Go Top In tmpv
...
*.comprobante1.enviarcorreocliente(.comprobante1.correo)
	...
	ocaja.cdcto = This.Serie + This.numero
	ocaja.nidprovedor = 0
	...
	ndolar = fe_gene.dola
	This.Idauto = This.IngresaResumenDctoIcbper()
	If This.Idauto < 1 Then
		Return 0
	Endif
...
*!*		Endif
	...
	If cform = 'C' Or cform = 'D' Then
		ocre = Newobject("ctasporcobrar", "d:\capass\modelos\ctasxcobrar.prg")
		...
		Endif
	Endif
	Select Idauto From tmpp Into Cursor resguias Group By Idauto
	m.Sw = 1
	Select resguias
	Scan All
		Text To lC Noshow Textmerge
	     UPDATE fe_rcom SET rcom_ccaj='C',rcom_idcg=<<this.Idauto>> where idauto=<<resguias.idauto>>
		Endtext
		If This.Ejecutarsql(lC) < 1 Then
			m.Sw = 0
			Exit
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	\ Select  `c`.`rcom_icbper` ,`c`.`rcom_mens` ,ifnull(m.Fevto,c.fech) As fvto,
	...
	\  `a`.`cant`        As `cant`, `a`.`Prec`        As `Prec`, `c`.`valor`, `c`.`igv`         As `igv`,
	\  `c`.`Impo`        As `Impo`, `c`.`fech`        As `fech`, `c`.`fecr` , `c`.`Form`        As `Form`,
	\  `c`.`Deta`        As `Deta`, `c`.`Exon`        As `Exon`, `c`.`Ndo2` , `c`.`rcom_entr`   As `rcom_entr`,
	\  `c`.`idcliente`   As `idclie`, `d`.`Razo`        As `Razo`, `d`.`nruc`        As `nruc`, `d`.`Dire` ,
	\  `d`.`ciud`        As `ciud`,  `d`.`ndni`        As `ndni`, `a`.`tipo`        As `tipo`, `c`.`Tdoc`,c.rcom_exon,
	...
	\  `b`.`uno`, `b`.`Dos`,b.tre,b.cua, (`b`.`uno` + `b`.`Dos`+b.tre+b.cua) As `TAlma`, `c`.`FUsua`,rcom_otro,kar_icbper,
...
*!*		Endif
  ...
  \Order By orden,Serie,Ndoc
	...
	If goapp.Proyecto = 'psysg' Or goapp.Proyecto = 'psysu' OR goapp.Proyecto='xsys5' Then
      \,kar_unid As Unid
	Endif
     \,a.Prec,If(c.Mone='S','Soles','Dólares') As Moneda,c.Tdoc,c.Ndoc,s.nomb As tienda,Month(fech) As mes,c.Mone,a.alma,p.Descri
	If goapp.Proyecto = 'psysg' Or goapp.Proyecto = 'xsys3' Or goapp.Proyecto = 'psysu' OR goapp.Proyecto='xsys5' Then
		\,kar_equi
	Endif
     \From fe_kar As a
     \inner Join fe_rcom  As c   On(c.Idauto=a.Idauto)
	...
	Do Case
	Case Month(This.Fecha) <> goapp.mes Or Year(This.Fecha) <> Val(goapp.Año)
		This.Cmensaje = "Fecha No Permitida Por el Sistema Mes Y Año Diferente al Actual"
		Return .F.
	Case This.nroformapago = 2 And  !esFechaValidafvto(This.Fechavto)
		This.Cmensaje = "Fecha de Vencimiento No Válida"
		Return .F.
	Case (fe_gene.fech - This.Fecha) > 3 And (This.Tdoc = '01' Or This.Tdoc = '03')
		This.Cmensaje = "Solo se permite con Máximo de 3 dias Anteriores"
		Return .F.
	Case  This.Fechavto <= This.Fecha And This.nroformapago = 2
		This.Cmensaje = "La Fecha de Vencimiento debe ser diferente de le fecha de Emisión "
		Return .F.
	...
	Case Left(This.cmensajerptasunat, 1) = "0"
		This.Cmensaje = "Este Documento Electrónico Ya esta Informado a SUNAT. NO es posible Modificarlo"
		Return .F.
	...
	Return .T.
	Endfunc
	Function listarparaguias(Ccursor)
	If !Pemstatus(goapp, 'cdatos', 5) Then
		AddProperty(goapp, 'cdatos', '')
	Endif
	If !Pemstatus(goapp, 'tiendas', 5) Then
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
    \Select Ndoc,fech,b.Razo,Mone,valor,igv,Impo,Idauto,b.nruc,b.ndni,b.Dire,b.ciud,a.idcliente,rcom_hash,rcom_arch,rcom_mens,a.idusua As idusuav
    \From fe_rcom As a inner Join fe_clie As b On b.idclie=a.idcliente
    \Where  a.Acti<>'I' And Month(fech)=<<This.nmes>> And Year(fech)=<<This.Naño>> And Tdoc In ("01","03")
	If goapp.Cdatos = 'S' Then
		If Empty(goapp.Tiendas) Then
	      \And a.codt=<<This.codt>>
		Else
	...
	Return 1
	Endfunc
	Function ActualizaResumenDctoConRehazo()
	lsql = 'ProActualizaCabeceraVtasconrechazo'
	goapp.npara1 = This.Tdoc
	goapp.npara2 = Left(This.formaPago, 1)
	goapp.npara3 = This.Serie + This.numero
	goapp.npara4 = This.Fecha
	goapp.npara5 = This.cmotivorechazo
	goapp.npara6 = This.valor
	goapp.npara7 = This.igv
	goapp.npara8 = This.Importe
	goapp.npara9 = This.NroGuia
	goapp.npara10 = Left(This.Moneda, 1)
	...
	goapp.npara17 = 1
	goapp.npara18 = This.codt
	goapp.npara19 = 0
	goapp.npara20 = 0
	goapp.npara21 = 0
	goapp.npara22 = 0
	goapp.npara23 = 0
	goapp.npara24 = This.Idauto
	Text To lparms Noshow
	...
	Return 1
	Endfunc
	Function listarechazos(Ccursor)
	fi = Cfechas(This.fechai)
	ff = Cfechas(This.fechaf)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
    \Select Ndoc,fech,c.Razo As cliente,Impo As Importe,p.Descri As producto,logk_unid As Unid,logk_cant As Cantidad,
	\Prec As Precio,rcom_more As motivo,v.nomv As Vendedor From fe_rcom As r
	\inner Join fe_clie As c On c.`idclie`=r.`idcliente`
	\inner Join fe_akardex As k On k.`logk_ida1`=r.`Idauto`
	\inner Join fe_vend As v On v.`idven`=k.`logk_codv`
	\inner Join fe_art As p On p.idart=k.`logk_idar` Where rcom_rech='S'And  r.fech Between '<<fi>>' And '<<ff>>' And r.Acti='A'
	\ Order By Ndoc
	Set Textmerge Off
	Set Textmerge To
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
	...
	Return 1
	Endfunc
	Function listarventascantidadmarcas(nidm, Ccursor)
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	\Select Descri As producto,tcant As Cantidad,v.nomv As Vendedor,aa.idart
    \From (Select  Sum(a.cant) As tcant,a.idart,Codv From fe_rcom As e
	...
	Endif
	If This.Vendedor > 0 Then
	  \ And Codv=<<This.Vendedor>>
	Endif
	\Group By a.idart,Codv) As yy
	\inner Join fe_art As aa On aa.idart=yy.idart
	...
	Return 1
	Endfunc
	Function mostrarventasparacanjespsysm(f1, f2, nm, Ccursor)
	If (f2 - f1) > 30 Then
		This.Cmensaje = "Máximo 30 Días para filtrar las Ventas"
		Return 0
	Endif
	...
	Text To lC Noshow Textmerge
		...
		FROM fe_rcom AS r
		INNER JOIN fe_kar AS k ON k.idauto=r.idauto
		WHERE tdoc='20' AND k.acti='A' AND r.acti='A' AND r.fech BETWEEN '<<dfi>>' AND '<<dff>>' and rcom_idtr=0 GROUP BY idart) AS s
		INNER JOIN fe_art AS a ON a.idart=s.idart
		INNER JOIN fe_fletes AS  f ON f.idflete=a.idflete,fe_gene AS g where importe>0
	Endtext
	Text To lcx Noshow Textmerge
		SELECT r.idauto FROM fe_rcom AS r
		INNER JOIN fe_kar AS k ON k.idauto=r.idauto
		WHERE tdoc='20' AND k.acti='A' AND r.acti='A'  AND r.fech BETWEEN '<<dfi>>' AND '<<dff>>'  and rcom_idtr=0  GROUP BY idauto
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	Return 1
	Endfunc
	Function registrarxserviciosconanticipo1()
	Local lC, lp
	lC			  = 'FunIngresaCabeceraVtasAnticipo'
	cur			  = "Xn"
	goapp.npara1  = This.Tdoc
	goapp.npara2  = Left(This.formaPago, 1)
	goapp.npara3  = This.Serie + This.numero
	goapp.npara4  = This.Fecha
	goapp.npara5  = This.Fecha
	goapp.npara6  = This.Detalle
	goapp.npara7  = This.valor
	goapp.npara8  = This.igv
	goapp.npara9  = This.Monto
	goapp.npara10 = ""
	goapp.npara11 = Left(This.Moneda, 1)
	goapp.npara12 = This.ndolar
	goapp.npara13 = This.vigv
	goapp.npara14 = 'S'
	goapp.npara15 = This.Codigo
	goapp.npara16 = This.Idanticipo
	goapp.npara17 = goapp.nidusua
	goapp.npara18 = This.Vendedor
	goapp.npara19 = This.codt
	goapp.npara20 = This.cta1
	goapp.npara21 = This.cta2
	goapp.npara22 = This.cta3
	goapp.npara23 = This.exonerado
	goapp.npara24 = This.detraccion
	goapp.npara25 = This.coddetraccion
	Text To lp Noshow
	(?goapp.npara1,?goapp.npara2,?goapp.npara3,?goapp.npara4,?goapp.npara5,?goapp.npara6,?goapp.npara7,?goapp.npara8,?goapp.npara9,
	?goapp.npara10,?goapp.npara11,?goapp.npara12,?goapp.npara13,?goapp.npara14,?goapp.npara15,?goapp.npara16,?goapp.npara17,
	?goapp.npara18,?goapp.npara19,?goapp.npara20,?goapp.npara21,?goapp.npara22,?goapp.npara23,?goapp.npara24,?goapp.npara25)
	...
	Return m.nid
	Endfunc
	Function ActualizarResumenDctovtasconAnticipo()
*This.Tdoc, Left(This.formaPago, 1), cndoc, This.Fecha, This.Fecha, This.Detalle, This.valor, This.igv, This.Monto, cguia, Left(This.Moneda, 1), ;
This.ndolar, This.vigv, 'S', This.Codigo, This.Idanticipo, goapp.nidusua, This.Vendedor, This.codt, This.cta1, This.cta2, This.cta3, This.exonerado, 0, This.detraccion, This.Idauto, This.coddetraccion
	lC			  = 'ProActualizarVtasAnticipo'
	cur			  = ""
	goapp.npara1  = This.Tdoc
	goapp.npara2  = Left(This.formaPago, 1)
	goapp.npara3  = This.Serie + This.numero
	...
	goapp.npara10 = Left(This.Moneda, 1)
	goapp.npara11 =  This.ndolar
	goapp.npara12 = This.vigv
	goapp.npara13 = "S"
	goapp.npara14 = This.Codigo
	goapp.npara15 = This.Idanticipo
	goapp.npara16 =  goapp.nidusua
	goapp.npara17 = This.Vendedor
	goapp.npara18 =  This.codt
	goapp.npara19 = This.cta1
	goapp.npara20 = This.cta2
	goapp.npara21 = This.cta3
	goapp.npara22 = This.exonerado
	goapp.npara23 = 0
	goapp.npara24 = This.detraccion
	goapp.npara25 =  This.Idauto
	goapp.npara26 = This.coddetraccion
	Text To lp Noshow
	(?goapp.npara1,?goapp.npara2,?goapp.npara3,?goapp.npara4,?goapp.npara5,?goapp.npara6,?goapp.npara7,?goapp.npara8,?goapp.npara9,
	?goapp.npara10,?goapp.npara11,?goapp.npara12,?goapp.npara13,?goapp.npara14,?goapp.npara15,?goapp.npara16,?goapp.npara17,
	...
	Return 1
	Endfunc
	Function rotacionResumidaProducto(ccoda)
	dfi = Cfechas(This.fechai)
	dff = Cfechas(This.fechaf)
	Ccursor = 'c_' + Sys(2015)
	If This.idsesion > 0 Then
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
    \Select Sum(cant) As cant
	\ From fe_kar As a
    \inner Join fe_rcom  As c   On(c.Idauto=a.Idauto)
    \inner Join fe_clie As b On (b.idclie=c.idcliente)
    \inner Join fe_sucu As s On s.idalma=c.codt
    \Where c.Acti<>'I' And a.Acti='A' And  c.fech Between '<<dfi>>' And '<<dff>>'
	If goapp.Proyecto = 'psysrx' Or goapp.Proyecto = 'psysr' Then
       \ And  idart='<<ccoda>>'
	Else
       \ And  idart=<<ccoda>>
	...
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	\Select  c.rcom_exon,  c.idusua ,  a.kar_comi ,  a.Codv,  a.Idauto,ifnull(m.Fevto,c.fech) As fvto,
	\c.codt      As alma,  a.kar_idco  As idcosto,  a.idkar     As idkar,  a.idart,  a.cant,
	\a.Prec      As Prec,  c.valor     As valor,  c.igv       As igv,  c.Impo      As Impo,  c.fech      As fech,
	\c.fecr      As fecr,  c.Form      As Form,  c.Deta      As Deta,  c.Exon      As Exon,  c.Ndo2      As Ndo2,
	\c.rcom_entr As rcom_entr,  c.idcliente As idclie,  d.Razo      As Razo,  d.nruc      As nruc,  d.Dire      As Dire,
	\d.ciud      As ciud,  d.ndni      As ndni,  a.tipo      As tipo,  c.Tdoc      As Tdoc,  c.Ndoc      As Ndoc,
	\c.dolar     As dolar,  c.Mone      As Mone,  b.Descri,
	\b.Unid      As Unid,  b.premay    As pre1,  b.peso, b.premen    As pre2,
	\ifnull(z.vend_idrv,0) As nidrv,  c.vigv      As vigv,  a.dsnc      As dsnc,rcom_mens,
	\a.dsnd      As dsnd,  a.gast      As gast,  c.codt      As codt,  b.pre3      As pre3,
	\b.cost      As costo,  b.uno       As uno,  b.Dos       As Dos,  (b.uno + b.Dos) As TAlma,  c.FUsua     As FUsua,
	\p.nomv      As Vendedor,  q.nomb      As Usuario,ifnull(s.seriep,"") As seriep,kar_cost
	...
	\Join fe_vend p        On p.idven = a.Codv
	\Join fe_usua q        On q.idusua = c.idusua
	\Left Join (Select vend_idau,vend_idrv From  `fe_rvendedor` Where vend_acti='A') `z`  On `z`.`vend_idau` = `c`.`Idauto`
	\Left Join (Select rser_seri As seriep,dser_idka From fe_rseries F
	\inner Join fe_dseries g On g.dser_idre=F.rser_idse Where g.dser_acti='A' And rser_acti='A') As s  On s.dser_idka=a.idkar
	\Left Join (Select rcre_idau,Min(c.Fevto) As Fevto From fe_rcred As r inner Join fe_cred As c On c.cred_idrc=r.rcre_idrc
    \Where rcre_acti='A' And Acti='A' And rcre_idau=<<This.Idauto>> Group By rcre_idau) As m On m.rcre_idau=c.Idauto
	...
	Return 1
	Endfunc
	Function utilidadpsysg(nidmar, nidcat, Ccursor)
	f1 = Cfechas(This.fechai)
	f2 = Cfechas(This.fechaf)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Set Textmerge On
	Set Textmerge To Memvar lx Noshow Textmerge
	\Select  coda,Descri,Unid,Cantidad,costo,
	\prVtas As PrecioPromedioVtas,prVtas*tcant As ImporteVentas,Cantidad*costo As ImporteCompras,
	\((prVtas*tcant)-(Cantidad*costo))/Cantidad As Utilidad,
	\(prVtas*Cantidad)-(Cantidad*costo) As margen,
	\((Round((prVtas*tcant)-(Cantidad*costo),2)/If(costo*Cantidad>0,costo*Cantidad,1))*100) As porcentaje
	\From (
	\Select z.coda,Descri,Unid,cant As Cantidad,
	\Cast((cant*If(a.tmon='S',a.prod_cost,prod_cost*g.dola))/cant As Decimal(12,2)) As costo,prVtas,tcant
	\From (
	\Select a.idart As coda,Cast(Sum(a.cant*a.kar_equi) As Decimal(12,2)) As cant,Cast(Sum(a.cant*a.kar_equi) As Decimal(12,2)) As tcant,
	\Cast(Sum(a.cant*a.Prec)/Sum(a.cant*a.kar_equi) As Decimal(12,2)) As prVtas
	\From fe_kar As a
	\inner Join fe_rcom As c On c.Idauto=a.Idauto
	\Where c.idcliente>0 And a.Acti='A' And c.Acti='A' And c.fech Between '<<f1>>' And '<<f2>>' And c.Exon<>'S'
	If This.codt > 0 Then
	  \ And a.alma=<<This.codt>>
	Endif
    \ Group By a.idart) As z
	\ inner Join fe_art As a On a.idart=z.coda, fe_gene As g
	If nidcat > 0 Then
	\ Where a.idcat=<<nidcat>>
		cw = 'S'
	Endif
	If nidmar > 0 Then
		If m.cw = 'S' Then
	    \ And a.idmar=<<nidmar>>
		Else
	    \ Where a.idmar=<<nidmar>>
		Endif
	Endif
	\) As xx
	Set Textmerge Off
	Set Textmerge To
	If This.EJECutaconsulta(lx, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	...
	Return  1
	Endfunc
	Function resumenventasxmescantidadpsystr(ntipo, Ccursor)
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
  	\Select Descri As producto,Unid As Unidad,m.dmar As marca,d.dcat As linea,
	...
	\Group By idart,fech
	\Order By fech) As x
	\inner Join fe_art As c On c.idart=x.idart
	\inner Join fe_mar m On m.idmar=c.idmar
	\inner Join fe_cat d On d.idcat=c.idcat
	...
	Return 1
	Endfunc
	Function resumenventasxmesmontospsystr(ntipo, Ccursor)
	Set Textmerge On
	Set Textmerge To Memvar lC Noshow Textmerge
	\Select Descri As producto,Unid As Unidad,m.dmar As marca,d.dcat As linea,
	\Sum(Case mes When 1 Then Impo Else 0 End) As 'Enero',
	...
	Return 1
	Endfunc
	Function ActualizaResumenDctovtascondetraccion1()
	cndoc = This.Serie + This.numero
	...
	goapp.npara23 = This.exonerado
	goapp.npara24 = 0
	goapp.npara25 = This.detraccion
	goapp.npara26 = This.Idauto
	goapp.npara27 = This.coddetraccion
	Text To lp Noshow
	(?goapp.npara1,?goapp.npara2,?goapp.npara3,?goapp.npara4,?goapp.npara5,?goapp.npara6,?goapp.npara7,?goapp.npara8,?goapp.npara9,
	?goapp.npara10,?goapp.npara11,?goapp.npara12,?goapp.npara13,?goapp.npara14,?goapp.npara15,?goapp.npara16,?goapp.npara17,
	...
	Return 1
	Endfunc
	Function AnalisisCLienteRentabilidad(Ccursor)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Text To lC Noshow Textmerge
	select nruc as RUC,razo as Cliente,TRIM(dire) as Direccion,fono,celu as Celular,
	sum(case mes when 1 then impo else 0 end) as 'Enero',
	sum(case mes when 1 then rta else 0 end) as 'Enero_g',
	sum(case mes when 2 then impo else 0 end) as 'Febrero',
	sum(case mes when 2 then rta else 0 end) as 'Febrero_g',
	sum(case mes when 3 then impo else 0 end) as 'Marzo',
	sum(case mes when 3 then rta else 0 end) as 'Marzo_g',
	sum(case mes when 4 then impo else 0 end) as 'Abril',
	sum(case mes when 4 then rta else 0 end) as 'Abril_g',
	sum(case mes when 5 then impo else 0 end) as 'Mayo',
	sum(case mes when 5 then rta else 0 end) as 'Mayo_g',
	sum(case mes when 6 then impo else 0 end) as 'Junio',
	sum(case mes when 6 then rta else 0 end) as 'Junio_g',
	sum(case mes when 7 then impo else 0 end) as 'Julio',
	sum(case mes when 7 then rta else 0 end) as 'Julio_g',
	sum(case mes when 8 then impo else 0 end) as 'Agosto',
	sum(case mes when 8 then rta else 0 end) as 'Agosto_g',
	sum(case mes when 9 then impo else 0 end) as 'Septiembre',
	sum(case mes when 9 then rta else 0 end) as 'Septiembre_g',
	sum(case mes when 10 then impo else 0 end) as 'Octubre',
    sum(case mes when 10 then rta else 0 end) as 'Octubre_g',
	sum(case mes when 11 then impo else 0 end) as 'Noviembre',
	sum(case mes when 11 then rta else 0 end) as 'Noviembre_g',
	sum(case mes when 12 then impo else 0 end) as 'Diciembre',
	sum(case mes when 12 then rta else 0 end) as 'Dicienbre_g',sum(impo) as Total,SUM(rta) as tganancia,idcliente
	from(
	SELECT MONTH(fech) AS mes,idcliente,CAST(SUM(IF(mone='S',impo,impo*dolar)) AS DECIMAL(12,2)) AS Impo,CAST(0 AS DECIMAL(12,2)) AS rta
	FROM fe_rcom r  WHERE acti='A' AND impo<>0 AND YEAR(fech)=<<this.Naño>> AND idcliente>0 GROUP BY idcliente,fech
	UNION ALL
	SELECT MONTH(fech) AS mes,idcliente,CAST(0 AS DECIMAL(12,2))AS impo,CAST(SUM((cant*IF(r.Mone='S',k.Prec,k.Prec*r.dolar))-(cant*k.kar_cost)) AS DECIMAL(12,2)) AS rta
	FROM fe_rcom AS r
	INNER JOIN fe_kar AS k ON k.idauto=r.`idauto`
	WHERE r.acti='A' AND impo<>0 AND YEAR(fech)=<<this.Naño>> AND k.acti='A' AND idcliente>0 GROUP BY idcliente,fech) as x
	inner join fe_clie as c on c.idclie=x.idcliente group by idcliente order by razo
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function AnalisisMarcaRentabilidad(Ccursor)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Text To lC Noshow Textmerge
	SELECT m.dmar as Marca,
	sum(case mes when 1 then impo else 0 end) as 'Enero',
	sum(case mes when 1 then rta else 0 end) as 'Enero_g',
	sum(case mes when 2 then impo else 0 end) as 'Febrero',
	sum(case mes when 2 then rta else 0 end) as 'Febrero_g',
	...
	sum(case mes when 11 then rta else 0 end) as 'Noviembre_g',
	sum(case mes when 12 then impo else 0 end) as 'Diciembre',
	sum(case mes when 12 then rta else 0 end) as 'Dicienbre_g',sum(impo) as Total,SUM(rta) as tganancia
	from (SELECT MONTH(fech) AS mes,c.idmar,cast(sum(if(mone='S',cant*k.prec,cant*k.prec*dolar)) as decimal(12,2)) as Impo,CAST(0 AS DECIMAL(12,2)) AS rta
	from fe_rcom r
	inner join fe_kar k on k.idauto=r.idauto
    inner join fe_art as c on c.idart=k.idart
    where k.acti='A' and r.acti='A'  and year(fech)=<<this.naño>>  and idcliente>0 group by c.idmar,r.fech
    union all
    SELECT MONTH(fech) AS mes,c.idmar,CAST(0 AS DECIMAL(12,2)) as impo,CAST(SUM((cant*IF(r.Mone='S',k.Prec,k.Prec*r.dolar))-(cant*k.kar_cost)) AS DECIMAL(12,2)) as rta
	from fe_rcom r
	inner join fe_kar k on k.idauto=r.idauto
    inner join fe_art as c on c.idart=k.idart
    where k.acti='A' and r.acti='A'  and year(fech)=<<this.naño>> and idcliente>0 group by c.idmar,r.fech
    ) as x
	inner join fe_mar m on m.idmar=x.idmar GROUP  BY x.idmar order by dmar
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function AnalisisLineaRentabilidad(Ccursor)
	If This.idsesion > 0 Then
		Set DataSession To This.idsesion
	Endif
	Text To lC Noshow Textmerge
	SELECT m.dcat as linea,
	sum(case mes when 1 then impo else 0 end) as 'Enero',
	sum(case mes when 1 then rta else 0 end) as 'Enero_g',
	sum(case mes when 2 then impo else 0 end) as 'Febrero',
	sum(case mes when 2 then rta else 0 end) as 'Febrero_g',
	...
	sum(case mes when 12 then impo else 0 end) as 'Diciembre',
	sum(case mes when 12 then rta else 0 end) as 'Dicienbre_g',sum(impo) as Total,SUM(rta) as tganancia
	from (SELECT MONTH(fech) AS mes,c.idcat,cast(sum(if(mone='S',cant*k.prec,cant*k.prec*dolar)) as decimal(12,2)) as Impo,CAST(0 AS DECIMAL(12,2)) AS rta
	from fe_rcom r
	inner join fe_kar k on k.idauto=r.idauto
    inner join fe_art as c on c.idart=k.idart
    where k.acti='A' and r.acti='A'  and year(fech)=<<this.naño>>  and idcliente>0 group by c.idcat,r.fech
    union all
    SELECT MONTH(fech) AS mes,c.idcat,CAST(0 AS DECIMAL(12,2)) as impo,CAST(SUM((cant*IF(r.Mone='S',k.Prec,k.Prec*r.dolar))-(cant*k.kar_cost)) AS DECIMAL(12,2)) as rta
	from fe_rcom r
	inner join fe_kar k on k.idauto=r.idauto
    inner join fe_art as c on c.idart=k.idart
    where k.acti='A' and r.acti='A'  and year(fech)=<<this.naño>> and idcliente>0 group by c.idcat,r.fech
    ) as x
	inner join fe_cat m on m.idcat=x.idcat GROUP  BY x.idcat order by dcat
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function Registroventasxsys(Ccursor)
	If (This.fechaf - This.fechai) > 366 Then
		This.Cmensaje = "Máximo 366 Días"
		Return 0
	Endif
	...
	\Sum(Case c.Nitem When 1 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As valor,
	\Sum(Case c.Nitem When 5 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As exonerado,
	\Sum(Case c.Nitem When 6 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As inafecto,
	\Sum(Case c.Nitem When 2 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As igv,
	\Sum(Case c.Nitem When 3 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As Impo,
	\Sum(Case c.Nitem When 4 Then If(a.Mone='S',c.Impo,Round(c.Impo*a.dolar,2)) Else 0 End) As pimpo,
	\a.idrven,ifnull(e.Ndoc,'') As Refe,ifnull(w.Tdoc,'00') As tref,
	...
	\Left Join fe_refe As e On(e.idrven=a.idrven)
    \Left Join fe_tdoc As w On w.idtdoc=e.idtdoc
	\Where a.Acti<>'I' And idalma=<<This.codt>> And a.fech Between '<<f1>>' And '<<f2>>'
	If Len(Alltrim(This.Serie)) > 0 Then
	 \ And Left(a.Ndoc,4)='<<this.serie>>'
	Endif
	\Group By a.idrven,e.idrefe
	...
	Text To lC Noshow
		select b.idrven as clave,b.idrven as idauto,x.venta as tc from fe_rven as b
	    inner join fe_mon as x on x.fech=b.fech
	    where b.acti<>'I' and month(b.fech)=?nm and year(b.fech)=?na
	Endtext
	Ccursor = 'c_' + Sys(2015)
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	...
	Return 1
	Endfunc
	Function ventasxvendedorpsysg(Ccursor)
	If !Pemstatus(goapp, 'proyecto', 5) Then
		AddProperty(goapp, 'proyecto', '')
	Endif
	f1 = Cfechas(This.fechai)
	...
	Set Textmerge To Memvar lC Noshow
	\Select a.kar_comi As comi,a.Idauto, e.Tdoc, e.Ndoc, e.fech, b.idart, a.cant, a.Prec, Round(a.cant * a.Prec, 2) As timporte,
	\e.Mone, a.alma, a.idart, b.idmar, c.nomv As nomb, e.Form,e.vigv As igv, a.Codv, Cast(e.dolar As Decimal(6,4)) As dola,
	\b.Descri, d.Razo, m.dmar As marca, d.nruc, d.ndni,prod_cost,b.prod_cod1,s.nomb As tienda,e.codt,a.kar_unid As Unid,
	\If(b.tmon='S',b.prod_cost,prod_cost*g.dola) As costo,e.idcliente
	\ From fe_rcom As e
	\inner Join fe_clie As d  On d.idclie = e.idcliente
	\Left Join fe_kar As a On a.Idauto = e.Idauto
	\Left Join fe_vend As c On c.idven = a.Codv
	\Left Join fe_art As  b On b.idart = a.idart
	\inner Join fe_sucu As s On s.idalma=e.codt
	\Left Join fe_mar As m On m.idmar = b.idmar,fe_gene As g
	\Where e.Acti <> 'I' And a.Acti <> 'I'  And e.fech  Between '<<f1>>' And '<<f2>>' And e.Exon<>'S'
	If This.Vendedor > 0 Then
	   \ And a.Codv=<<This.Vendedor>>
	Endif
	If This.codt > 0 Then
	...
	Text To lC Noshow
	    ...
	    where a.acti='A' and month(a.fech)=?nmes and year(a.fech)=?na and a.idcliente>0
	Endtext
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	Endif
	...
	If 	This.GRabarCambios() < 1 Then
		Return 0
	Endif
	This.Cmensaje = "Se Aplico Tipo de Cambio Correctamente a las Ventas"
	Return 1
	Endfunc
	Function IngresaResumenDcto()
	lC = 'FunIngresaCabeceraCV'
	cur = "Xn"
	npara1 = This.Tdoc
	npara2 = This.formaPago
	npara3 = This.Serie + This.numero
	npara4 = This.Fecha
	npara5 = This.Fecha
	npara6 = This.Detalle
	npara7 = This.valor
	npara8 = This.igv
	npara9 = This.Importe
	npara10 = This.NroGuia
	npara11 = This.Moneda
	npara12 = This.ndolar
	npara13 = This.vigv
	npara14 = This.Ctipovta
	npara15 = This.Codigo
	npara16 = 'V'
	npara17 = This.Usuario
	npara18 = 1
	npara19 = This.codt
	npara20 = This.cta1
	npara21 = This.cta2
   ...
   (?npara1,?npara2,?npara3,?npara4,?npara5,?npara6,?npara7,?npara8,?npara9,?npara10,?npara11,?npara12,?npara13,?npara14,?npara15,?npara16,?npara17,?npara18,?npara19,?npara20,?npara21,?npara22,?npara23,?npara24)
	...
	Return m.nid
	Endfunc
	Function PermiteAnularVentaspsysn(NAuto, dFecha, cTdoc)
	ocaja = Newobject("cajae", "d:\capass\modelos\cajae.prg")
	oguia = Newobject("guiaremision", "d:\capass\modelos\guiasremision.prg")
	If _Screen.octasxcobrar.VerificaSitienePagos(NAuto) < 1 Then
		This.Cmensaje = _Screen.octasxcobrar.Cmensaje
		Return 0
	Endif
	If ocaja.PermiteIngresoACaja(dFecha) < 1 Then
		This.Cmensaje = ocaja.Cmensaje
		Return 0
	Endif
	If cTdoc = '07' Then
		Return 1
	Endif
	If cTdoc = '01' Or cTdoc = '03' Then
		If oguia.TieneDespachos(NAuto) < 1 Then
			This.Cmensaje = oguia.Cmensaje
			Return 0
		Endif
	Endif
	If This.verificasitienenotadecredito(NAuto) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function PermiteAnularVentas(NAuto, dFecha, cTdoc)
	If !Pemstatus(goapp, 'proyecto', 5) Then
		AddProperty(goapp, 'proyecto', '')
	Endif
	If goapp.Proyecto = 'xsys' Then
		If This.VerificaBloqueoVentaxsys(dFecha) < 1 Then
			Return 0
		Endif
	Endif
	If _Screen.octasxcobrar.VerificaSitienePagos(NAuto) < 1 Then
	...
	Return 1
	Endfunc
	Function verificasitienenotadecredito(nidauto)
	Ccursor = 'c_' + Sys(2015)
	Text To lC Noshow Textmerge Pretext 7
	...
	Select (Ccursor)
	If Idauto > 0 Then
		This.Cmensaje = 'Documento Ya Tiene Nota de Crédito No se Puede Anular '
		Return  0
	Endif
	Return  1
	Endfunc
	Function IngresaResumenDctoIcbper()
	If  Type('oempresa') = 'U' Then
		cnruc = fe_gene.nruc
	Else
		cnruc = oempresa.nruc
	Endif
	If cnruc = '20602510621'
		np15 = 0
	Else
		np15 = 'V'
	Endif
	...
	npara11 = This.ndolar
	npara12 = This.vigv
	npara13 = This.Ctipovta
	npara14 = This.Codigo
	npara15 = m.np15
	npara16 = This.Usuario
	npara17 = This.codt
	npara18 = This.cta1
	npara19 = This.cta2
	npara20 = This.cta3
	npara21 = 0
	npara22 = This.montopercepcion
	npara23 = This.nicbper
	Text To lp Noshow
   (?npara1,?npara2,?npara3,?npara4,?npara5,?npara6,?npara7,?npara8,?npara9,?npara10,?npara11,?npara12,?npara13,?npara14,?npara15,?npara16,?npara17,?npara18,?npara19,?npara20,?npara21,?npara22,?npara23)
	Endtext
	nid = This.EJECUTARf(lC, lp, cur)
	If m.nid <  1  Then
		Return 0
	...
	Return m.nid
	Endfunc
	Function ActualizarventaconGuiaIntinerante(nid, nidguia)
	Text To lC Noshow Textmerge
	   UPDATE fe_rcom SET rcom_idgu=<<m.nidguia>> where idauto=<<m.nid>>
	Endtext
	If This.Ejecutarsql(lC) < 1 Then
		Return 0
	Endif
	Return 1
	Endfunc
	Function VerificaBloqueoVentaxsys(dFecha)
	nmes = Month(dFecha)
	nyear = Year(dFecha)
	Text To lC Noshow Textmerge
	 SELECT idauto FROM fe_rcom
	 WHERE MONTH(fecr)=<<m.nmes>> AND YEAR(fecr)=<<m.nyear>> AND acti='A' AND rcom_bloq='C' AND idcliente>0 limit 1
	Endtext
	Ccursor = 'c_' + Sys(2015)
	If This.EJECutaconsulta(lC, Ccursor) < 1 Then
		Return 0
	...
	Select (Ccursor)
	If Idauto > 0 Then
		This.Cmensaje = ' Están bloqueados los Registros'
		Return 0
	Endif
	Return 1
	Endfunc
	...
	m.rpta = oser.Inicializar(This, 'Ventasxsys')
	If m.rpta < 1 Then
	...
	ocaja.Cdetalle = "Venta  " + ocaja.Ndoc
	ocaja.nidcta = This.cta3
	ocaja.ndebe = This.Monto
	ocaja.Cmoneda = This.Moneda
	...
	If This.IniciaTransaccion() < 1 Then
		Return 0
	Endif
	lC = "FUNINGRESARVENTAS"
	npara1 = This.Codigo
	npara2 = This.idtdoc
	npara3 = This.formaPago
	npara4 = This.Serie + This.numero
	npara5 = This.Fecha
	npara6 = This.Fecha
	npara7 = This.Fechavto
	npara8 = This.Moneda
	npara9 = This.ndolar
	npara10 = This.vigv
	npara11 = goapp.nidusua
	npara12 = This.valor
	npara13 = This.igv
	npara14 = This.Monto
	npara15 = This.cta1
	npara16 = This.cta2
	npara17 = This.cta3
	npara18 = This.montopercepcion
	npara19 = This.cta4
	npara20 = goapp.tienda
	npara21 = This.cta5
	npara22 = This.exonerado
	npara23 = This.cta6
	npara24 = This.inafecta
	Text To lp Noshow
   (?npara1,?npara2,?npara3,?npara4,?npara5,?npara6,?npara7,?npara8,?npara9,?npara10,?npara11,?npara12,?npara13,?npara14,?npara15,?npara16,?npara17,?npara18,?npara19,?npara20,?npara21,?npara22,?npara23,?npara24)
	Endtext
	m.nidrven = This.EJECUTARf(lC, lp, "")
	If m.nidrven <  1  Then
		This.DEshacerCambios()
		Return 0
	Endif
	If This.formaPago = 'E' Then
		ocaja.NAuto = m.nidrven
		If ocaja.IngresaDatosLCajaEFectivoxsys() < 1 Then
			This.DEshacerCambios()
			This.Cmensaje = ocaja.Cmensaje
			Return
		Endif
	Endif
	_Screen.octasxcobrar.Idauto = nidrven
	If Left(This.formaPago, 1) = 'C' Then
		_Screen.octasxcobrar.nidclie = This.Codigo
		_Screen.octasxcobrar.cndoc = This.Serie + This.numero
		_Screen.octasxcobrar.cestado = 'C'
		_Screen.octasxcobrar.Cmoneda  = Left(This.Moneda, 1)
		_Screen.octasxcobrar.dFech = This.Fecha
		_Screen.octasxcobrar.Fechavto = This.Fechavto
		_Screen.octasxcobrar.tipodcto = Left(This.tipodcto, 1)
		_Screen.octasxcobrar.cndoc = This.Serie + This.numero
		_Screen.octasxcobrar.nimpo = This.Monto
		_Screen.octasxcobrar.Codv = 1
		_Screen.octasxcobrar.nimpoo = This.Monto
		...
		Endif
	Endif
	If This.nidautoref > 0 Then
		onotas = Newobject("notacreditovtas", "d:\capass\modelos\notacreditovtas.prg")
		If onotas.registrarxsys(m.nidrven) < 1 Then
			This.DEshacerCambios()
			This.Cmensaje = onotas.Cmensaje
			Return 0
		Endif
		onotas = Null
	Endif
	If This.Idserie > 0 Then
		ocorr = Newobject("Correlativo", "d:\capass\modelos\correlativos.prg")
		ocorr.Nsgte = This.Nsgte
		ocorr.Idserie = This.Idserie
		ocorr.Ndoc = This.Serie + This.numero
		If ocorr.GeneraCorrelativo() < 1 Then
			This.DEshacerCambios()
			This.Cmensaje = ocorr.Cmensaje
			Return 0
	...
	ndclie = This.Codigo
	cTdoc = This.idtdoc
	cforma = Left(This.formaPago, 1)
	cndoc = This.Serie + This.numero
	dFecha = This.Fecha
	Fevto = This.Fechavto
	Cmon = Left(This.Moneda, 1)
	ndolar = This.ndolar
	nidrven = This.Idauto
	ocaja = Newobject("cajae", "d:\capass\modelos\cajae.prg")
	ocaja.dFecha = This.Fecha
	...
	Endif
	Text To lC Noshow
	UPDATE fe_rven SET idclie=?nidclie,idtdoc=?ctdoc,form=?cforma,ndoc=?cndoc,fech=?dfecha,fevto=?dfevto,
	 mone=?cmon,dolar=?ndolar,idalma=?nidalma where idrven=?nidrven
	Endtext
	If This.Ejecutarsql(lC) < 1 Then
		This.DEshacerCambios()
		Return 0
	Endif
	If ocaja.desactivaropcaja1(This.Idauto) < 1 Then
		This.DEshacerCambios()
		This.Cmensaje = Thisform.ocaja.Cmensaje
		Return
	Endif
	If This.ActualizaRegistrosCuentas() < 1 Then
		This.DEshacerCambios()
		Return 0
	Endif
	If Left(This.formaPago, 1) = 'E' Then
		...
		Endif
	Endif
	If _Screen.octasxcobrar.DesactivaRcreditos(This.Idauto, goapp.nidusua) < 1 Then
		This.DEshacerCambios()
		This.Cmensaje = _Screen.octasxcobrar.Cmensaje
		Return 0
	Endif
	...
	If This.nidautoref > 0 Then
		onotas = Newobject("notacreditovtas", "d:\capass\modelos\notacreditovtas.prg")
		If onotas.registrarxsys(This.Idauto) < 1 Then
			This.DEshacerCambios()
			Return 0
		Endif
		onotas = Null
	...
	Return 1
	Endfunc
	Function ActualizaRegistrosCuentas()
	lC = "actualizacuentasv"
	goapp.npara1 = This.valor
	goapp.npara2 = This.igv
	goapp.npara3 = This.Monto
	goapp.npara4 = This.cta1
	goapp.npara5 = This.cta2
	goapp.npara6 = This.cta3
	goapp.npara7 = This.idrven1
	goapp.npara8 = This.idrven2
	goapp.npara9 = This.idrven3
	goapp.npara10 = This.ctipocta1
	goapp.npara11 = This.ctipocta2
	goapp.npara12 = This.ctipocta3
	goapp.npara13 = This.montopercepcion
	goapp.npara14 = This.cta4
	goapp.npara15 = This.idrven4
	goapp.npara16 = This.ctipocta4
	goapp.npara17 = This.exonerado
	goapp.npara18 = This.cta5
	goapp.npara19 = This.idrven5
	goapp.npara20 = This.ctipocta5
	goapp.npara21 = This.inafecta
	goapp.npara22 = This.cta6
	goapp.npara23 = This.Idrven6
	goapp.npara24 = This.ctipocta6
	Text To lp Noshow
   (?goapp.npara1,?goapp.npara2,?goapp.npara3,?goapp.npara4,?goapp.npara5,?goapp.npara6,?goapp.npara7,?goapp.npara8,?goapp.npara9,?goapp.npara10,?goapp.npara11,?goapp.npara12,?goapp.npara13,?goapp.npara14,?goapp.npara15,?goapp.npara16,?goapp.npara17,?goapp.npara18,?goapp.npara19,?goapp.npara20,?goapp.npara21,?goapp.npara22,?goapp.npara23,?goapp.npara24)
	Endtext
	If  This.EJECUTARP(lC, lp, '') < 1 Then
	...
	Return 1
	Endfunc
	Function IngresaDocumentoElectronicoconretencion()
	Local lC, lp
	*.Tdoc, Left(.cmbFORMA.Value, 1),  cndoc, dfvta, Alltrim(Left(.Txtdetalle.Value, 110)), nvalor, nigv, nTotal, cguia, Left(.cmbMONEDA.Value, 1), ;
		.txtdolAR.Value, fe_gene.igv, 'k', .txtCodigo.Value, 'V', goApp.nidusua, goApp.Tienda, nidcta1, nidcta2, nidcta3, m.nvuelto, 0, 0, 0, nretencion
	lC			  = 'FuningresaDocumentoElectronico'
	cur			  = "Xn"
	npara1  = this.Tdoc
	npara2  = LEFT(this.formaPago,1)
	npara3  = this.Serie+this.numero
	npara4  = this.Fecha
	npara5  = this.Detalle
	npara6  = this.valor
	...
	npara8  = this.Importe
	npara9  = this.NroGuia
	npara10 = LEFT(this.Moneda,1)
	npara11 = this.ndolar
	npara12 = fe_gene.igv
	npara13 = 'k'
	npara14 = this.Codigo
	npara15 = 'V'
	npara16 = goApp.nidusua
	npara17 = this.codt
	npara18 = this.cta1
	npara19 = this.cta2
	npara20 = this.cta3
	npara21 = this.porcentajeretencion
	npara22 = this.exonerado
	npara23 = 0
	npara24 = 0
	npara25 = this.montoretencion
	Text To lp Noshow
