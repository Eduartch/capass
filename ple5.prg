...
Go Top In tmpv
Select * From tmpv Into Table Addbs(Sys(5) + Sys(2003)) + 't.dbf'
Endproc
********************************
...
Endproc
*********************************
Procedure Mensaje_Espera
Lparameters lcMess
If Type("lcMess") = "L"
	Return .F.
Endif
Wait Window lcMess At Srows() / 2, (Scols() / 2 - (Len(lcMess) / 2)) Noclear
Endproc
************************************
...
Local lC, lk
Set Procedure To d:\capass\modelos\inventarios, d:\capass\modelos\appsysven, d:\capass\modelos\importadatos Additive
oinv = Createobject("inventarios")
oapp = Createobject("appsysven")
oimp = Createobject("importadatos")
If oapp.Consultarfechaservidor('ff') < 1 Then
	Aviso(oapp.Cmensaje)
	Return
Endif
cpropiedad = "Otrodia"
...
Else
	...
	fe	   = Cfechas(dfecha)
	If oapp.ActualizaFechaServidor(ff.fechaservidor, ninicioenvios) < 1 Then
		Aviso(oapp.Cmensaje)
		Return
	Endif
	nm = nmes
	goApp.datosg = ""
	dATOSGLOBALES()
	If oimp.consultartcdata(nmes, nanio, 'ya') < 1 Then
		Aviso(oimp.Cmensaje)
		Return
	Endif
...
Else
	...
	dATOSGLOBALES()
	nm = nmes
	If oimp.consultartcdata(m.nmes, m.Na, 'ya') < 1 Then
		Aviso(oimp.Cmensaje)
		Return
	Endif
	If REgdvto("ya") = 0 Then
...
Case  odvto.Vdvto = '2'
	...
	Otherwise
		If AnulaTransaccionConMotivo('', '', 'V', odcto.Idauto, odcto.Idusua, 'S', Ctod(odcto.fech), goApp.uauto, odcto.detalle) = 0 Then
			Aviso("NO Se Anulo Correctamente de la Base de Datos")
			Return 0
		Endif
	Endcase
...
Set Procedure To;
	...
	d:\capass\modelos\inventarios,;
	d:\capass\modelos\conceptoscaja Additive
...
AddProperty(_Screen, 'oconceptos', Createobject("conceptoscaja"))
Endfunc
*****************************
...
Lparameters taArray, tcValor, tnColumna
Local i
For i = 1 To Alen(taArray, 1)
	If taArray[i, tnColumna] == tcValor
		Return i   && devuelve la fila
	Endif
Endfor
Return 0
Endfunc
...
Endfunc
************************
Function PdfToBase64(tcRutaPDF)
Local loXML, loNode, lcBinario
If Not File(tcRutaPDF)
	Return ""
Endif
* Leer archivo en modo binario
lcBinario = Filetostr(tcRutaPDF)
* Crear objeto XML
loXML = Createobject("MSXML2.DOMDocument.6.0")
loNode = loXML.createElement("base64")
loNode.DataType = "bin.base64"
loNode.nodeTypedValue = lcBinario
Return loNode.Text
Endfunc
*************************
Function Base64ToPdf(tcBase64, tcRutaDestino)
Local loXML, loNode, lcBinario
loXML = Createobject("MSXML2.DOMDocument.6.0")
loNode = loXML.createElement("base64")
loNode.DataType = "bin.base64"
loNode.Text = tcBase64
